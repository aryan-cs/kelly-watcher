...
class WalletDiscoveryRuntimeSyncTests(unittest.TestCase):
    def _run_until_wallet_discovery_scan(
        self,
        *,
        ...
        class _FakeScheduler:
            ...
            def start(self) -> None:
                callback = self.jobs.get("wallet_discovery_scan")
                if callback is None:
                    raise AssertionError("wallet_discovery_scan job was not registered")
                callback()
                raise _StopAfterWalletDiscovery("stop after wallet discovery scan")
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                with ExitStack() as stack:
                    ...
                    stack.enter_context(patch("kelly_watcher.main.wallet_discovery_enabled", return_value=True))
                    stack.enter_context(
                        patch("kelly_watcher.main.wallet_discovery_scan_interval_seconds", return_value=300)
                    )
                    stack.enter_context(patch("kelly_watcher.main.send_alert"))
                    with self.assertRaisesRegex(_StopAfterWalletDiscovery, "stop after wallet discovery scan"):
                        main.main()
            finally:
    ...
    def test_wallet_discovery_scan_immediately_refreshes_runtime_after_auto_promotion(self) -> None:
        watchlist_stub, tracker_stub, refresh_cache, dashboard_server = self._run_until_wallet_discovery_scan(
            discovery_summary={
                "ok": True,
        ...
        tracker_stub.replace_wallets.assert_called_once_with(["0xold", "0xnew"])
        self.assertIn(call(["0xold", "0xnew"]), refresh_cache.call_args_list)
        dashboard_server.stop.assert_called_once()
        tracker_stub.close.assert_called_once()

    def test_wallet_discovery_scan_does_not_refresh_runtime_without_new_promotions(self) -> None:
        watchlist_stub, tracker_stub, refresh_cache, dashboard_server = self._run_until_wallet_discovery_scan(
            discovery_summary={
                "ok": True,
                "finished_at": 1_700_000_000,
        ...
        tracker_stub.replace_wallets.assert_not_called()
        self.assertNotIn(call(["0xold", "0xnew"]), refresh_cache.call_args_list)
        dashboard_server.stop.assert_called_once()
        tracker_stub.close.assert_called_once()
