...
function formatSettingsPercent(value, digits = 2) {
...
function formatSegmentSummaryValue(value) {
    if (value == null)
        return '-';
...
function segmentSummaryLinesFromJson(raw) {
    if (!raw)
        return [];
...
const CONFIG_BLURBS = {
    ...
    DATA_API_REQUEST_RATE_PER_SECOND: 'Caps Polymarket data API request throughput.',
    DATA_API_REQUEST_BURST: 'Allows short controlled bursts above the steady data API rate.',
    WALLET_TRADE_FETCH_WORKERS: 'Controls parallel wallet trade-feed fetches.',
    ENRICHMENT_FETCH_WORKERS: 'Controls parallel metadata and orderbook fetches.',
    MAX_MARKET_HORIZON: 'Limits how far out copied markets may resolve.',
    MAX_SOURCE_TRADE_AGE: 'Strict source-trade age limit for near-resolution markets.',
    MAX_SOURCE_TRADE_AGE_FAR: 'Source-trade age limit for farther-out markets.',
    SOURCE_TRADE_AGE_FAR_MARKET_HORIZON: 'Minimum time to resolution before the far source-age limit applies.',
    ...
    HEURISTIC_ALLOWED_ENTRY_PRICE_BANDS: 'Optionally restricts heuristic trades to specific entry-price bands.',
    ADAPTIVE_HEURISTIC_ENTRY_PRICE_ENABLED: 'Lets recent resolved shadow evidence choose heuristic entry-price bands.',
    ADAPTIVE_HEURISTIC_ENTRY_PRICE_LOOKBACK: 'Controls how far back adaptive entry-band evidence is read.',
    ADAPTIVE_HEURISTIC_ENTRY_PRICE_REFRESH: 'Controls how often adaptive entry-price bands are recomputed.',
    ADAPTIVE_HEURISTIC_ENTRY_PRICE_MIN_SAMPLES: 'Requires this many evidence rows before adaptive bands activate.',
    ADAPTIVE_HEURISTIC_ENTRY_PRICE_MIN_BAND_SAMPLES: 'Requires this many rows inside one band before selecting it.',
    ADAPTIVE_HEURISTIC_ENTRY_PRICE_MIN_AVG_RETURN: 'Requires this weighted average return before selecting a band.',
...
export function Settings({ editor }) {
    ...
    const recoverDbDangerBlurb = !editor.dangerConfirm && selectedDangerAction?.id === 'recover_db'
    ...
    const restartShadowDangerBlurb = !editor.dangerConfirm && selectedDangerAction?.id === 'restart_shadow' && shadowRestartPending
        ? shadowRestartMessage
        : '';
    const dangerBlurb = shadowRestartPending && !editor.dangerConfirm && selectedDangerAction?.id === 'live_trading'
    ...
    const startupBlockedHelperLines = startupRecoveryOnly
        ? wrapText(startupBlockReason
            ...
            : 'Recovery-only mode: backend startup is blocked until Recover DB or Restart Shadow completes.', dangerContentWidth)
        : [];
    ...
    const shadowReadinessSummary = useMemo(() => {
        ...
        if (requireTotalShadowHistory && minShadowResolved > 0) {
            summaryParts.push(`all-time base ${shadowHistoryTotalReady ? 'ready' : 'need'} ${formatNumber(resolvedShadowTradeCount)}/${formatNumber(minShadowResolved)}`);
        }
        else {
        ...
        if (minShadowSincePromotion > 0) {
            ...
            summaryParts.push(`since-promotion ${shadowHistoryReady ? 'ready' : 'need'} ${formatNumber(resolvedShadowSincePromotion)}/${formatNumber(minShadowSincePromotion)} since ${promotionLabel}`);
        }
        else {
    ...
    const shadowHistoryEpochDetailWrappedLines = shadowHistoryEpochDetailLines.flatMap((line) => wrapText(line, Math.max(24, panelContentWidth)));
    ...
    const routedShadowTotalResolved = Math.max(0, Number(state.routed_shadow_total_resolved || (routedShadowRoutedResolved + routedShadowLegacyResolved) || 0));
    ...
    const routedShadowPerformanceDetailWrappedLines = routedShadowPerformanceDetailLines.flatMap((line) => wrapText(line, Math.max(24, panelContentWidth)));
    ...
    const routedShadowDetailWrappedLines = routedShadowDetailLines.flatMap((line) => wrapText(line, Math.max(24, panelContentWidth)));
    ...
    const dbIntegrityLines = dbIntegrityKnown && dbIntegrityMessage
    ...
    const dbRecoveryDetailWrappedLines = dbRecoveryDetailLines.flatMap((line) => wrapText(line, Math.max(24, panelContentWidth)));
    ...
    const dbRecoveryShadowMigratedEvaluation = isMigratedRecoveryShadowStatus(dbRecoveryShadowStatusRaw)
    ...
    const dbRecoveryShadowDetailWrappedLines = dbRecoveryShadowDetailLines.flatMap((line) => wrapText(line, Math.max(24, panelContentWidth)));
    ...
    const environmentWalletBoxWidth = stacked
        ? '100%'
        : Math.max(36, environmentRowWidth - Number(environmentStatsBoxWidth) - environmentRowGap);
    const environmentStatsContentWidth = typeof environmentStatsBoxWidth === 'number'
        ? Math.max(24, environmentStatsBoxWidth - 4)
        : panelContentWidth;
    const environmentWalletContentWidth = typeof environmentWalletBoxWidth === 'number'
        ? Math.max(24, environmentWalletBoxWidth - 4)
        : panelContentWidth;
    const environmentStatsCapacity = Math.max(1, environmentBoxHeight - 3);
    const envRows = envData.rows.length > environmentStatsCapacity
        ? envData.rows.slice(0, Math.max(0, environmentStatsCapacity - 1))
        : envData.rows;
    const hiddenEnvRowCount = Math.max(0, envData.rows.length - envRows.length);
    const walletContentCapacity = Math.max(1, environmentBoxHeight - 3);
    const walletRowsAfterHeader = Math.max(0, walletContentCapacity - 2);
    const maxWalletLines = envData.watchedWallets.length <= walletRowsAfterHeader
        ? envData.watchedWallets.length
        : Math.max(0, walletRowsAfterHeader - 1);
    ...
    const walletTableWidth = Math.max(1, environmentWalletContentWidth);
    ...
    return (React.createElement(InkBox, { flexDirection: "column", width: "100%" },
        ...
        React.createElement(InkBox, { flexDirection: stacked ? 'column' : 'row', width: "100%" },
            React.createElement(Box, { title: "Editable Config", width: stacked ? '100%' : configBoxWidth, accent: true },
                ...
                React.createElement(InkBox, { flexDirection: "column", marginTop: 1 },
                    ...
                    configHelperLines.map((line, index) => (React.createElement(Text, { key: `config-status-${index}`, color: configHelperColor }, line))))),
        ...
        React.createElement(InkBox, { flexDirection: stacked ? 'column' : 'row', width: "100%" },
            ...
            !stacked ? React.createElement(InkBox, { width: environmentRowGap }) : React.createElement(InkBox, { height: 1 }),
            React.createElement(Box, { title: `Watched Wallets (${envData.watchedWallets.length})`, width: environmentWalletBoxWidth, height: environmentBoxHeight }, envData.watchedWallets.length ? (React.createElement(React.Fragment, null,
                visibleWallets.length ? (React.createElement(React.Fragment, null,
                    ...
                    visibleWallets.map((wallet, index) => (React.createElement(InkBox, { key: `${wallet}-${index}`, width: "100%", flexShrink: 0 },
                        ...
                        walletAddressWidth > 0 ? (React.createElement(React.Fragment, null,
                            React.createElement(Text, null, " "),
                            React.createElement(Text, { color: theme.white }, fit(wallet, walletAddressWidth)))) : null))))) : (React.createElement(Text, { color: theme.dim }, truncate('Terminal is too short to list wallets.', environmentWalletContentWidth))),
                hiddenWalletCount > 0 ? (React.createElement(InkBox, { width: "100%", flexShrink: 0 },
                    React.createElement(Text, { color: theme.dim }, truncate(`... and ${hiddenWalletCount} more`, environmentWalletContentWidth)))) : null)) : (React.createElement(Text, { color: theme.dim }, "No watched wallets configured."))))));
}
