...
interface RetrainRunRow {
  ...
  val_selected_trades: number | null
  val_total_pnl: number | null
...
interface ReplaySearchSummaryRow {
  ...
  daily_guard_window_penalty: number | null
  live_guard_window_penalty: number | null
  daily_guard_restart_window_penalty: number | null
  live_guard_restart_window_penalty: number | null
  open_exposure_penalty: number | null
  window_end_open_exposure_penalty: number | null
  avg_window_end_open_exposure_penalty: number | null
  carry_window_penalty: number | null
  carry_restart_window_penalty: number | null
  resolved_share_penalty: number | null
  resolved_size_share_penalty: number | null
  worst_window_resolved_share_penalty: number | null
  worst_window_resolved_size_share_penalty: number | null
  mode_resolved_share_penalty: number | null
  mode_resolved_size_share_penalty: number | null
  mode_worst_window_resolved_share_penalty: number | null
  mode_worst_window_resolved_size_share_penalty: number | null
  mode_active_window_accepted_share_penalty: number | null
  mode_active_window_accepted_size_share_penalty: number | null
  worst_active_window_accepted_penalty: number | null
  worst_active_window_accepted_size_penalty: number | null
  mode_worst_active_window_accepted_penalty: number | null
  mode_worst_active_window_accepted_size_penalty: number | null
  mode_loss_penalty: number | null
  mode_inactivity_penalty: number | null
  mode_accepted_window_count_penalty: number | null
  mode_accepted_window_share_penalty: number | null
  mode_non_accepting_active_window_streak_penalty: number | null
  mode_non_accepting_active_window_episode_penalty: number | null
  mode_accepting_window_accepted_share_penalty: number | null
  mode_accepting_window_accepted_size_share_penalty: number | null
  mode_top_two_accepting_window_accepted_share_penalty: number | null
  mode_top_two_accepting_window_accepted_size_share_penalty: number | null
  mode_accepting_window_accepted_concentration_index_penalty: number | null
  mode_accepting_window_accepted_size_concentration_index_penalty: number | null
  ...
  top_two_accepting_window_accepted_share_penalty: number | null
  top_two_accepting_window_accepted_size_share_penalty: number | null
  accepting_window_accepted_concentration_index_penalty: number | null
  accepting_window_accepted_size_concentration_index_penalty: number | null
  ...
  wallet_size_concentration_penalty: number | null
  market_size_concentration_penalty: number | null
  entry_price_band_size_concentration_penalty: number | null
  time_to_close_band_size_concentration_penalty: number | null
...
function retrainRunHumanSummary(row: RetrainRunRow): string {
...
function replaySearchScopeLabel(scope: string | null | undefined): string {
  const normalized = String(scope || '').trim().toLowerCase()
  if (!normalized) return '-'
  if (normalized === 'shadow_only') return 'shadow only'
  return normalized.replace(/_/g, ' ')
}
...
function replaySearchStatusColor(status: string | null | undefined): string {
  const normalized = String(status || '').trim().toLowerCase()
  if (!normalized) return theme.dim
  if (normalized === 'completed') return theme.green
  if (normalized === 'running') return theme.blue
  if (normalized === 'already_running') return theme.yellow
  if (normalized === 'failed') return theme.red
  return theme.white
}

function replayPromotionStatusLabel(status: string | null | undefined): string {
  const normalized = String(status || '').trim().toLowerCase()
  if (!normalized) return '-'
  if (normalized === 'applied') return 'Applied'
  if (normalized === 'failed') return 'Failed'
  if (normalized === 'disabled') return 'Disabled'
  if (normalized === 'skipped_live_mode') return 'Skip live'
  if (normalized === 'skipped_no_feasible_candidate') return 'Skip no cand'
  if (normalized === 'skipped_no_config') return 'Skip no cfg'
  if (normalized === 'skipped_unchanged') return 'Skip same'
  if (normalized === 'skipped_score_delta') return 'Skip score'
  if (normalized === 'skipped_pnl_delta') return 'Skip pnl'
  return normalized.replace(/_/g, ' ')
}

function replayPromotionStatusColor(status: string | null | undefined): string {
  const normalized = String(status || '').trim().toLowerCase()
  if (!normalized) return theme.dim
  if (normalized === 'applied') return theme.green
  if (normalized === 'failed') return theme.red
  if (normalized === 'disabled') return theme.dim
  if (normalized.startsWith('skipped_')) return theme.yellow
  return theme.white
}
...
function replaySearchCompletionDetail(value: string | null | undefined): string {
...
function replaySearchModeMixSummary(
  ...
  try {
    ...
    const parsedEntries = Object.entries(rawSummary as Record<string, unknown>)
      ...
      .filter((entry): entry is {mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number; activeWindowCount: number; acceptedWindowCount: number; acceptedWindowShare: number; maxNonAcceptingActiveWindowStreak: number; nonAcceptingActiveWindowEpisodeCount: number; maxAcceptingWindowAcceptedShare: number; maxAcceptingWindowAcceptedSizeShare: number; topTwoAcceptingWindowAcceptedShare: number; topTwoAcceptingWindowAcceptedSizeShare: number; acceptingWindowAcceptedConcentrationIndex: number; acceptingWindowAcceptedSizeConcentrationIndex: number} => Boolean(entry))
      .filter((entry) => {
    ...
    const entries: Array<{mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number; activeWindowCount: number; acceptedWindowCount: number; acceptedWindowShare: number; maxNonAcceptingActiveWindowStreak: number; nonAcceptingActiveWindowEpisodeCount: number; maxAcceptingWindowAcceptedShare: number; maxAcceptingWindowAcceptedSizeShare: number; topTwoAcceptingWindowAcceptedShare: number; topTwoAcceptingWindowAcceptedSizeShare: number; acceptingWindowAcceptedConcentrationIndex: number; acceptingWindowAcceptedSizeConcentrationIndex: number}> = []
...
function replaySearchCurrentModeEvidenceSummary(
  ...
  try {
    ...
    const parsedEntries = Object.entries(rawSummary as Record<string, unknown>)
      ...
      .filter((entry): entry is {mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number; winRate: number | null; activeWindowCount: number; acceptedWindowCount: number; acceptedWindowShare: number; maxNonAcceptingActiveWindowStreak: number; nonAcceptingActiveWindowEpisodeCount: number; maxAcceptingWindowAcceptedShare: number; maxAcceptingWindowAcceptedSizeShare: number; topTwoAcceptingWindowAcceptedShare: number; topTwoAcceptingWindowAcceptedSizeShare: number; acceptingWindowAcceptedConcentrationIndex: number; acceptingWindowAcceptedSizeConcentrationIndex: number} => Boolean(entry))
      .filter((entry) => {
        if (entry.mode === 'heuristic') return enabled.heuristic
        if (entry.mode === 'xgboost') return enabled.xgboost
        return true
    ...
    const entryByMode = new Map(parsedEntries.map((entry) => [entry.mode, entry]))
    const entries: Array<{mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number; winRate: number | null; activeWindowCount: number; acceptedWindowCount: number; acceptedWindowShare: number; maxNonAcceptingActiveWindowStreak: number; nonAcceptingActiveWindowEpisodeCount: number; maxAcceptingWindowAcceptedShare: number; maxAcceptingWindowAcceptedSizeShare: number; topTwoAcceptingWindowAcceptedShare: number; topTwoAcceptingWindowAcceptedSizeShare: number; acceptingWindowAcceptedConcentrationIndex: number; acceptingWindowAcceptedSizeConcentrationIndex: number}> = []
    if (enabled.heuristic) {
      entries.push(entryByMode.get('heuristic') ?? {
        mode: 'heuristic',
        acceptedCount: 0,
...
function replaySearchModeFloorSummary(
  ...
  try {
    ...
    if (!enabled.heuristic) {
      ...
      if (minHeuristicResolvedSizeShare > 0) parts.push(`heur sz-cov>=${formatPct(minHeuristicResolvedSizeShare, 0)}`)
      ...
      if (maxHeuristicNonAcceptingActiveWindowEpisodes >= 0) parts.push(`heur acc-runs<=${formatCount(maxHeuristicNonAcceptingActiveWindowEpisodes)}`)
      ...
      if (maxHeuristicTopTwoAcceptingWindowAcceptedShare > 0) parts.push(`heur top2-acc<=${formatPct(maxHeuristicTopTwoAcceptingWindowAcceptedShare, 0)}`)
      if (maxHeuristicTopTwoAcceptingWindowAcceptedSizeShare > 0) parts.push(`heur top2-acc$<=${formatPct(maxHeuristicTopTwoAcceptingWindowAcceptedSizeShare, 0)}`)
      if (maxHeuristicAcceptingWindowAcceptedConcentrationIndex > 0) parts.push(`heur acc-ci<=${formatPct(maxHeuristicAcceptingWindowAcceptedConcentrationIndex, 0)}`)
      if (maxHeuristicAcceptingWindowAcceptedSizeConcentrationIndex > 0) parts.push(`heur acc-ci$<=${formatPct(maxHeuristicAcceptingWindowAcceptedSizeConcentrationIndex, 0)}`)
      ...
      if (mixModesEnabled && maxHeuristicActiveWindowAcceptedShare > 0) parts.push(`heur acc-mix<=${formatPct(maxHeuristicActiveWindowAcceptedShare, 0)}`)
      if (mixModesEnabled && maxHeuristicActiveWindowAcceptedSizeShare > 0) parts.push(`heur acc-mix$<=${formatPct(maxHeuristicActiveWindowAcceptedSizeShare, 0)}`)
    }
    if (!enabled.xgboost) {
      ...
      if (minXgboostResolvedSizeShare > 0) parts.push(`model sz-cov>=${formatPct(minXgboostResolvedSizeShare, 0)}`)
      ...
      if (maxXgboostNonAcceptingActiveWindowEpisodes >= 0) parts.push(`model acc-runs<=${formatCount(maxXgboostNonAcceptingActiveWindowEpisodes)}`)
      ...
      if (maxXgboostTopTwoAcceptingWindowAcceptedShare > 0) parts.push(`model top2-acc<=${formatPct(maxXgboostTopTwoAcceptingWindowAcceptedShare, 0)}`)
      if (maxXgboostTopTwoAcceptingWindowAcceptedSizeShare > 0) parts.push(`model top2-acc$<=${formatPct(maxXgboostTopTwoAcceptingWindowAcceptedSizeShare, 0)}`)
      if (maxXgboostAcceptingWindowAcceptedConcentrationIndex > 0) parts.push(`model acc-ci<=${formatPct(maxXgboostAcceptingWindowAcceptedConcentrationIndex, 0)}`)
      if (maxXgboostAcceptingWindowAcceptedSizeConcentrationIndex > 0) parts.push(`model acc-ci$<=${formatPct(maxXgboostAcceptingWindowAcceptedSizeConcentrationIndex, 0)}`)
      ...
      if (mixModesEnabled && minXgboostActiveWindowAcceptedShare > 0) parts.push(`model acc-mix>=${formatPct(minXgboostActiveWindowAcceptedShare, 0)}`)
      if (mixModesEnabled && minXgboostActiveWindowAcceptedSizeShare > 0) parts.push(`model acc-mix$>=${formatPct(minXgboostActiveWindowAcceptedSizeShare, 0)}`)
    }
...
function replaySearchHasParticipation(payload: Record<string, unknown>): boolean {
  if (Number(payload.accepted_count || 0) > 0) return true
  if (Number(payload.accepted_size_usd || 0) > 0) return true
  if (Number(payload.resolved_count || 0) > 0) return true
...
function replaySearchActiveWindowCountFromPayload(payload: Record<string, unknown>): number {
  ...
  if (windowCount <= 1) return replaySearchHasParticipation(payload) ? 1 : 0
...
function replaySearchAcceptedWindowShareFromPayload(payload: Record<string, unknown>): number {
...
function replaySearchPositiveWindowCountFromPayload(payload: Record<string, unknown>): number {
  if (payload.positive_window_count != null) {
    return Math.max(Number(payload.positive_window_count || 0), 0)
  }
  const windowCount = Number(payload.window_count || 0)
  if (windowCount <= 1) return Number(payload.total_pnl_usd || 0) > 0 ? 1 : 0
  return 0
}

function replaySearchNegativeWindowCountFromPayload(payload: Record<string, unknown>): number {
...
function replaySearchWorstActiveWindowAcceptedCountFromPayload(payload: Record<string, unknown>): number {
...
function replaySearchWorstActiveWindowAcceptedSizeUsdFromPayload(payload: Record<string, unknown>): number {
...
function replaySearchLegacyModeActiveWindowMixFallback(
  modeValue: number,
  totalValue: number,
  windowCount: number
): {min: number; max: number} {
  if (totalValue <= 0) return {min: 1, max: 0}
  if (windowCount <= 1) {
    const share = modeValue / totalValue
    return {min: share, max: share}
  }
  if (modeValue <= 0) return {min: 0, max: 0}
  if (modeValue >= totalValue) return {min: 1, max: 1}
  return {min: 0, max: 1}
}

function replaySearchModeActiveWindowAcceptedShareFromPayload(
  payload: Record<string, unknown>,
  mode: 'heuristic' | 'xgboost',
  windowCount: number,
  ...
  const fallback = replaySearchLegacyModeActiveWindowMixFallback(
    Number(payload.accepted_count || 0),
    totalAcceptedCount,
    windowCount
  )
...
function replaySearchModeActiveWindowAcceptedSizeShareFromPayload(
  payload: Record<string, unknown>,
  mode: 'heuristic' | 'xgboost',
  windowCount: number,
  totalAcceptedSizeUsd: number
): number {
  const key = mode === 'heuristic' ? 'max_active_window_accepted_size_share' : 'min_active_window_accepted_size_share'
  if (payload[key] != null) return Number(payload[key] || 0)
  const fallback = replaySearchLegacyModeActiveWindowMixFallback(
    Number(payload.accepted_size_usd || 0),
    totalAcceptedSizeUsd,
    windowCount
  )
  return mode === 'heuristic' ? fallback.max : fallback.min
}
...
function replaySearchHasProvenWorstWindowPnlFromPayload(
  payload: Record<string, unknown>,
  windowCountOverride?: number
): boolean {
  if (payload.has_proven_worst_window_pnl != null) return Boolean(payload.has_proven_worst_window_pnl)
  if (payload.worst_window_pnl_usd != null) return true
...
function replaySearchWorstActiveWindowResolvedShareFromPayload(payload: Record<string, unknown>): number {
  ...
  const acceptedCount = Number(payload.accepted_count || 0)
  if (acceptedCount <= 0) return 1
  const resolvedCount = Number(payload.resolved_count || 0)
  const windowCount = Number(payload.window_count || 0)
  const exactShare = resolvedCount / acceptedCount
  if (windowCount <= 1) return exactShare
  return 0
}

function replaySearchWorstActiveWindowResolvedSizeShareFromPayload(payload: Record<string, unknown>): number {
...
function replaySearchMaxNonAcceptingActiveWindowStreakFromPayload(payload: Record<string, unknown>): number {
  ...
  if (acceptedWindowCount <= 0) return 0
...
function replaySearchModeAcceptedWindowShareFromPayload(payload: Record<string, unknown>, windowCount: number): number {
...
function replaySearchModeWorstActiveWindowAcceptedCountFromPayload(payload: Record<string, unknown>, windowCount: number): number {
  if (payload.worst_accepting_window_accepted_count != null) {
    return Number(payload.worst_accepting_window_accepted_count || 0)
  }
  if (payload.worst_active_window_accepted_count != null) {
...
function replaySearchModeWorstActiveWindowAcceptedSizeUsdFromPayload(payload: Record<string, unknown>, windowCount: number): number {
  if (payload.worst_accepting_window_accepted_size_usd != null) {
    return Number(payload.worst_accepting_window_accepted_size_usd || 0)
  }
  if (payload.worst_active_window_accepted_size_usd != null) {
...
function replaySearchModeNonAcceptingActiveWindowEpisodeCountFromPayload(payload: Record<string, unknown>, windowCount: number): number {
  if (payload.non_accepting_active_window_episode_count != null) {
    return Math.max(Number(payload.non_accepting_active_window_episode_count || 0), 0)
  }
...
function replaySearchCarryRestartWindowShareFromPayload(payload: Record<string, unknown>): number {
  if (payload.carry_restart_window_share != null) return Number(payload.carry_restart_window_share || 0)
...
function replaySearchCarryWindowSummaryFromPayload(payload: Record<string, unknown>): string {
  const activeWindowCount = replaySearchActiveWindowCountFromPayload(payload)
  const explicitCount = payload.carry_window_count == null ? null : Math.max(Number(payload.carry_window_count || 0), 0)
  if (explicitCount != null) {
    if (activeWindowCount > 0) return `${formatCount(explicitCount)}/${formatCount(activeWindowCount)}`
    if (explicitCount > 0) return 'yes'
    return ''
  }
  return replaySearchCarryWindowShareFromPayload(payload) > 0 ? 'yes' : ''
}

function replaySearchCarryRestartSummaryFromPayload(payload: Record<string, unknown>): string {
...
function replaySearchDailyGuardRestartWindowShareFromPayload(payload: Record<string, unknown>): number {
  if (payload.daily_guard_restart_window_share != null) return Number(payload.daily_guard_restart_window_share || 0)
...
function replaySearchLiveGuardRestartWindowShareFromPayload(payload: Record<string, unknown>): number {
  if (payload.live_guard_restart_window_share != null) return Number(payload.live_guard_restart_window_share || 0)
...
function replaySearchScoreBreakdownSummary(raw: string | null | undefined): string {
  ...
  try {
    ...
    if (Math.abs(dailyGuardWindowPenaltyUsd) > 1e-9) parts.push(`d-freq ${formatDollar(-dailyGuardWindowPenaltyUsd)}`)
    if (Math.abs(liveGuardWindowPenaltyUsd) > 1e-9) parts.push(`p-freq ${formatDollar(-liveGuardWindowPenaltyUsd)}`)
    if (Math.abs(dailyGuardRestartWindowPenaltyUsd) > 1e-9) parts.push(`d-rst ${formatDollar(-dailyGuardRestartWindowPenaltyUsd)}`)
    if (Math.abs(liveGuardRestartWindowPenaltyUsd) > 1e-9) parts.push(`p-rst ${formatDollar(-liveGuardRestartWindowPenaltyUsd)}`)
    ...
    if (Math.abs(avgWindowEndOpenExposurePenaltyUsd) > 1e-9) parts.push(`carry-avg ${formatDollar(-avgWindowEndOpenExposurePenaltyUsd)}`)
    if (Math.abs(carryWindowPenaltyUsd) > 1e-9) parts.push(`c-freq ${formatDollar(-carryWindowPenaltyUsd)}`)
    if (Math.abs(carryRestartWindowPenaltyUsd) > 1e-9) parts.push(`c-rst ${formatDollar(-carryRestartWindowPenaltyUsd)}`)
    ...
    if (Math.abs(topTwoAcceptingWindowAcceptedSharePenaltyUsd) > 1e-9) parts.push(`top2-acc ${formatDollar(-topTwoAcceptingWindowAcceptedSharePenaltyUsd)}`)
    if (Math.abs(topTwoAcceptingWindowAcceptedSizeSharePenaltyUsd) > 1e-9) parts.push(`top2-acc$ ${formatDollar(-topTwoAcceptingWindowAcceptedSizeSharePenaltyUsd)}`)
    if (Math.abs(acceptingWindowAcceptedConcentrationIndexPenaltyUsd) > 1e-9) parts.push(`acc-ci ${formatDollar(-acceptingWindowAcceptedConcentrationIndexPenaltyUsd)}`)
    ...
    if (Math.abs(modeAcceptedWindowSharePenaltyUsd) > 1e-9) parts.push(`m-acc-freq ${formatDollar(-modeAcceptedWindowSharePenaltyUsd)}`)
    if (Math.abs(modeNonAcceptingActiveWindowStreakPenaltyUsd) > 1e-9) parts.push(`m-acc-gap ${formatDollar(-modeNonAcceptingActiveWindowStreakPenaltyUsd)}`)
    if (Math.abs(modeNonAcceptingActiveWindowEpisodePenaltyUsd) > 1e-9) parts.push(`m-acc-runs ${formatDollar(-modeNonAcceptingActiveWindowEpisodePenaltyUsd)}`)
    if (Math.abs(modeAcceptingWindowAcceptedSharePenaltyUsd) > 1e-9) parts.push(`m-top-acc ${formatDollar(-modeAcceptingWindowAcceptedSharePenaltyUsd)}`)
    if (Math.abs(modeAcceptingWindowAcceptedSizeSharePenaltyUsd) > 1e-9) parts.push(`m-top-acc$ ${formatDollar(-modeAcceptingWindowAcceptedSizeSharePenaltyUsd)}`)
    if (Math.abs(modeTopTwoAcceptingWindowAcceptedSharePenaltyUsd) > 1e-9) parts.push(`m-top2-acc ${formatDollar(-modeTopTwoAcceptingWindowAcceptedSharePenaltyUsd)}`)
    if (Math.abs(modeTopTwoAcceptingWindowAcceptedSizeSharePenaltyUsd) > 1e-9) parts.push(`m-top2-acc$ ${formatDollar(-modeTopTwoAcceptingWindowAcceptedSizeSharePenaltyUsd)}`)
    if (Math.abs(modeAcceptingWindowAcceptedConcentrationIndexPenaltyUsd) > 1e-9) parts.push(`m-acc-ci ${formatDollar(-modeAcceptingWindowAcceptedConcentrationIndexPenaltyUsd)}`)
    if (Math.abs(modeAcceptingWindowAcceptedSizeConcentrationIndexPenaltyUsd) > 1e-9) parts.push(`m-acc-ci$ ${formatDollar(-modeAcceptingWindowAcceptedSizeConcentrationIndexPenaltyUsd)}`)
    ...
    if (Math.abs(walletSizeConcentrationPenaltyUsd) > 1e-9) parts.push(`wallet$ ${formatDollar(-walletSizeConcentrationPenaltyUsd)}`)
    if (Math.abs(marketSizeConcentrationPenaltyUsd) > 1e-9) parts.push(`market$ ${formatDollar(-marketSizeConcentrationPenaltyUsd)}`)
    if (Math.abs(entryPriceBandSizeConcentrationPenaltyUsd) > 1e-9) parts.push(`band$ ${formatDollar(-entryPriceBandSizeConcentrationPenaltyUsd)}`)
    if (Math.abs(timeToCloseBandSizeConcentrationPenaltyUsd) > 1e-9) parts.push(`hzn$ ${formatDollar(-timeToCloseBandSizeConcentrationPenaltyUsd)}`)
    return parts.join(' | ')
  } catch {
    return '-'
  }
...
function replaySearchScoreDriftSummary(
  ...
  if (Math.abs(dailyGuardWindowDelta) > 1e-9) parts.push(`d-freq ${formatDollar(dailyGuardWindowDelta)}`)
  if (Math.abs(liveGuardWindowDelta) > 1e-9) parts.push(`p-freq ${formatDollar(liveGuardWindowDelta)}`)
  if (Math.abs(dailyGuardRestartWindowDelta) > 1e-9) parts.push(`d-rst ${formatDollar(dailyGuardRestartWindowDelta)}`)
  if (Math.abs(liveGuardRestartWindowDelta) > 1e-9) parts.push(`p-rst ${formatDollar(liveGuardRestartWindowDelta)}`)
  ...
  if (Math.abs(avgCarryDelta) > 1e-9) parts.push(`carry-avg ${formatDollar(avgCarryDelta)}`)
  if (Math.abs(carryWindowDelta) > 1e-9) parts.push(`c-freq ${formatDollar(carryWindowDelta)}`)
  if (Math.abs(carryRestartWindowDelta) > 1e-9) parts.push(`c-rst ${formatDollar(carryRestartWindowDelta)}`)
  ...
  if (Math.abs(topTwoAcceptingWindowAcceptedShareDelta) > 1e-9) parts.push(`top2-acc ${formatDollar(topTwoAcceptingWindowAcceptedShareDelta)}`)
  if (Math.abs(topTwoAcceptingWindowAcceptedSizeShareDelta) > 1e-9) parts.push(`top2-acc$ ${formatDollar(topTwoAcceptingWindowAcceptedSizeShareDelta)}`)
  if (Math.abs(acceptingWindowAcceptedConcentrationIndexDelta) > 1e-9) parts.push(`acc-ci ${formatDollar(acceptingWindowAcceptedConcentrationIndexDelta)}`)
  ...
  if (Math.abs(modeAcceptedWindowShareDelta) > 1e-9) parts.push(`m-acc-freq ${formatDollar(modeAcceptedWindowShareDelta)}`)
  if (Math.abs(modeNonAcceptingActiveWindowStreakDelta) > 1e-9) parts.push(`m-acc-gap ${formatDollar(modeNonAcceptingActiveWindowStreakDelta)}`)
  if (Math.abs(modeNonAcceptingActiveWindowEpisodeDelta) > 1e-9) parts.push(`m-acc-runs ${formatDollar(modeNonAcceptingActiveWindowEpisodeDelta)}`)
  if (Math.abs(modeAcceptingWindowAcceptedShareDelta) > 1e-9) parts.push(`m-top-acc ${formatDollar(modeAcceptingWindowAcceptedShareDelta)}`)
  if (Math.abs(modeAcceptingWindowAcceptedSizeShareDelta) > 1e-9) parts.push(`m-top-acc$ ${formatDollar(modeAcceptingWindowAcceptedSizeShareDelta)}`)
  if (Math.abs(modeTopTwoAcceptingWindowAcceptedShareDelta) > 1e-9) parts.push(`m-top2-acc ${formatDollar(modeTopTwoAcceptingWindowAcceptedShareDelta)}`)
  if (Math.abs(modeTopTwoAcceptingWindowAcceptedSizeShareDelta) > 1e-9) parts.push(`m-top2-acc$ ${formatDollar(modeTopTwoAcceptingWindowAcceptedSizeShareDelta)}`)
  if (Math.abs(modeAcceptingWindowAcceptedConcentrationIndexDelta) > 1e-9) parts.push(`m-acc-ci ${formatDollar(modeAcceptingWindowAcceptedConcentrationIndexDelta)}`)
  if (Math.abs(modeAcceptingWindowAcceptedSizeConcentrationIndexDelta) > 1e-9) parts.push(`m-acc-ci$ ${formatDollar(modeAcceptingWindowAcceptedSizeConcentrationIndexDelta)}`)
  ...
  if (Math.abs(walletSizeDelta) > 1e-9) parts.push(`wallet$ ${formatDollar(walletSizeDelta)}`)
  if (Math.abs(marketSizeDelta) > 1e-9) parts.push(`market$ ${formatDollar(marketSizeDelta)}`)
  if (Math.abs(entryBandSizeDelta) > 1e-9) parts.push(`band$ ${formatDollar(entryBandSizeDelta)}`)
  if (Math.abs(horizonSizeDelta) > 1e-9) parts.push(`hzn$ ${formatDollar(horizonSizeDelta)}`)
  return parts.join(' | ')
}
...
function replaySearchTraderConcentrationSummary(
  ...
  sizePenalty: number | null | undefined
): ReplaySearchTraderConcentrationSummary {
  const parse = (raw: string | null | undefined): {count: number | null; peakCount: number | null; acceptedShare: number | null; absPnlShare: number | null; sizeShare: number | null} => {
    if (!raw) return {count: null, peakCount: null, acceptedShare: null, absPnlShare: null, sizeShare: null}
    try {
      const parsed = JSON.parse(raw)
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return {count: null, peakCount: null, acceptedShare: null, absPnlShare: null, sizeShare: null}
  ...
  const current = parse(currentRaw)
  const limits = (() => {
    if (!constraintsRaw) return {count: 0, accepted: 0, pnl: 0, size: 0}
    try {
      const parsed = JSON.parse(constraintsRaw)
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return {count: 0, accepted: 0, pnl: 0, size: 0}
      return {
        ...
        size: Number((parsed as Record<string, unknown>).max_top_trader_size_share || 0)
      }
    } catch {
  ...
  if (resolvedSizePenalty > 0) parts.push(`size pen ${resolvedSizePenalty.toFixed(2)}x`)
  const overLimit =
  ...
  const limits = (() => {
    ...
    try {
      ...
      return {
        ...
        size: Number((parsed as Record<string, unknown>).max_top_market_size_share || 0)
      }
    } catch {
      return {count: 0, accepted: 0, pnl: 0, size: 0}
    }
  ...
  const limits = (() => {
    ...
    try {
      ...
      return {
        ...
        size: Number((parsed as Record<string, unknown>).max_top_entry_price_band_size_share || 0)
      }
    } catch {
      return {count: 0, accepted: 0, pnl: 0, size: 0}
    }
  ...
  const limits = (() => {
    ...
    try {
      ...
      return {
        ...
        size: Number((parsed as Record<string, unknown>).max_top_time_to_close_band_size_share || 0)
      }
    } catch {
      return {count: 0, accepted: 0, pnl: 0, size: 0}
    }
...
function replaySearchPauseGuardSummary(
  ...
  dailyGuardWindowPenalty: number | null | undefined,
  liveGuardWindowPenalty: number | null | undefined,
  dailyGuardRestartWindowPenalty: number | null | undefined,
  liveGuardRestartWindowPenalty: number | null | undefined
): ReplaySearchPauseGuardSummary {
  const parseState = (
    raw: string | null | undefined
  ): {
    rejectShare: number | null
    dailyGuardWindowShare: number | null
    liveGuardWindowShare: number | null
    dailyGuardRestartWindowShare: number | null
    liveGuardRestartWindowShare: number | null
    initialBankrollUsd: number | null
  } | null => {
    if (!raw) return null
    try {
      const parsed = JSON.parse(raw)
  ...
  const bestDailyGuardWindowShare = bestState?.dailyGuardWindowShare ?? null
  const currentDailyGuardWindowShare = currentState?.dailyGuardWindowShare ?? null
  ...
  const bestDailyGuardRestartWindowShare = bestState?.dailyGuardRestartWindowShare ?? null
  const currentDailyGuardRestartWindowShare = currentState?.dailyGuardRestartWindowShare ?? null
  const bestLiveGuardRestartWindowShare = bestState?.liveGuardRestartWindowShare ?? null
  const currentLiveGuardRestartWindowShare = currentState?.liveGuardRestartWindowShare ?? null
  const {
    maxShare,
    maxDailyGuardWindowShare,
    maxLiveGuardWindowShare,
    maxDailyGuardRestartWindowShare,
    maxLiveGuardRestartWindowShare
  } = (() => {
    if (!constraintsRaw) {
      return {
        maxShare: 0,
    ...
    try {
      ...
      return {
        ...
        maxDailyGuardWindowShare: Number((parsed as Record<string, unknown>).max_daily_guard_window_share || 0),
        maxLiveGuardWindowShare: Number((parsed as Record<string, unknown>).max_live_guard_window_share || 0),
        maxDailyGuardRestartWindowShare: Number((parsed as Record<string, unknown>).max_daily_guard_restart_window_share || 0),
        maxLiveGuardRestartWindowShare: Number((parsed as Record<string, unknown>).max_live_guard_restart_window_share || 0)
      }
    } catch {
      return {
        maxShare: 0,
  ...
  if (
    ...
    if (bestDailyGuardWindowShare != null) {
      const penaltyCost = formatPenaltyCost(bestState?.initialBankrollUsd ?? null, resolvedDailyGuardWindowPenalty, bestDailyGuardWindowShare)
    ...
    if (bestDailyGuardRestartWindowShare != null) {
      const penaltyCost = formatPenaltyCost(bestState?.initialBankrollUsd ?? null, resolvedDailyGuardRestartWindowPenalty, bestDailyGuardRestartWindowShare)
    ...
    if (bestLiveGuardRestartWindowShare != null) {
      const penaltyCost = formatPenaltyCost(bestState?.initialBankrollUsd ?? null, resolvedLiveGuardRestartWindowPenalty, bestLiveGuardRestartWindowShare)
  ...
  if (
    ...
    if (currentDailyGuardWindowShare != null) {
      const penaltyCost = formatPenaltyCost(currentState?.initialBankrollUsd ?? null, resolvedDailyGuardWindowPenalty, currentDailyGuardWindowShare)
    ...
    if (currentDailyGuardRestartWindowShare != null) {
      const penaltyCost = formatPenaltyCost(currentState?.initialBankrollUsd ?? null, resolvedDailyGuardRestartWindowPenalty, currentDailyGuardRestartWindowShare)
    ...
    if (currentLiveGuardRestartWindowShare != null) {
      const penaltyCost = formatPenaltyCost(currentState?.initialBankrollUsd ?? null, resolvedLiveGuardRestartWindowPenalty, currentLiveGuardRestartWindowShare)
  ...
  if (
    ...
    if (maxDailyGuardWindowShare > 0) limitParts.push(`max d-freq ${formatPct(maxDailyGuardWindowShare, 0)}`)
    if (maxLiveGuardWindowShare > 0) limitParts.push(`max p-freq ${formatPct(maxLiveGuardWindowShare, 0)}`)
    if (maxDailyGuardRestartWindowShare > 0) limitParts.push(`max d-rst ${formatPct(maxDailyGuardRestartWindowShare, 0)}`)
    if (maxLiveGuardRestartWindowShare > 0) limitParts.push(`max p-rst ${formatPct(maxLiveGuardRestartWindowShare, 0)}`)
  ...
  if (
    ...
    if (resolvedDailyGuardWindowPenalty > 0) penaltyParts.push(`d-freq pen ${resolvedDailyGuardWindowPenalty.toFixed(2)}x`)
    if (resolvedLiveGuardWindowPenalty > 0) penaltyParts.push(`p-freq pen ${resolvedLiveGuardWindowPenalty.toFixed(2)}x`)
    if (resolvedDailyGuardRestartWindowPenalty > 0) penaltyParts.push(`d-rst pen ${resolvedDailyGuardRestartWindowPenalty.toFixed(2)}x`)
    if (resolvedLiveGuardRestartWindowPenalty > 0) penaltyParts.push(`p-rst pen ${resolvedLiveGuardRestartWindowPenalty.toFixed(2)}x`)
...
function replaySearchModePenaltySummary(row: ReplaySearchSummaryRow | undefined): string {
  ...
  if (modeResolvedSizeSharePenalty > 0) parts.push(`sz-cov ${modeResolvedSizeSharePenalty.toFixed(2)}x`)
  ...
  if (modeAcceptedWindowSharePenalty > 0) parts.push(`acc-freq ${modeAcceptedWindowSharePenalty.toFixed(2)}x`)
  if (modeNonAcceptingActiveWindowStreakPenalty > 0) parts.push(`acc-gap ${modeNonAcceptingActiveWindowStreakPenalty.toFixed(2)}x`)
  if (modeNonAcceptingActiveWindowEpisodePenalty > 0) parts.push(`acc-runs ${modeNonAcceptingActiveWindowEpisodePenalty.toFixed(2)}x`)
  if (modeAcceptingWindowAcceptedSharePenalty > 0) parts.push(`top-acc ${modeAcceptingWindowAcceptedSharePenalty.toFixed(2)}x`)
  if (modeAcceptingWindowAcceptedSizeSharePenalty > 0) parts.push(`top-acc$ ${modeAcceptingWindowAcceptedSizeSharePenalty.toFixed(2)}x`)
  if (modeTopTwoAcceptingWindowAcceptedSharePenalty > 0) parts.push(`top2-acc ${modeTopTwoAcceptingWindowAcceptedSharePenalty.toFixed(2)}x`)
  if (modeTopTwoAcceptingWindowAcceptedSizeSharePenalty > 0) parts.push(`top2-acc$ ${modeTopTwoAcceptingWindowAcceptedSizeSharePenalty.toFixed(2)}x`)
  if (modeAcceptingWindowAcceptedConcentrationIndexPenalty > 0) parts.push(`acc-ci ${modeAcceptingWindowAcceptedConcentrationIndexPenalty.toFixed(2)}x`)
  if (modeAcceptingWindowAcceptedSizeConcentrationIndexPenalty > 0) parts.push(`acc-ci$ ${modeAcceptingWindowAcceptedSizeConcentrationIndexPenalty.toFixed(2)}x`)
  return parts.length ? parts.join(' | ') : 'none'
}
...
function replaySearchCurrentModeRiskSummary(
  ...
  try {
    ...
    const windowCount = Number(currentParsed?.window_count || 0)
    ...
    for (const [mode, prefix, shareKey, shareDirection] of [
      ...
      if (minResolvedSizeShare > 0) {
        hasActiveGuard = true
        if (resolvedSizeShare < minResolvedSizeShare) breaches.push(`${prefix} sz-cov ${formatPct(resolvedSizeShare, 0)}<${formatPct(minResolvedSizeShare, 0)}`)
      }
      if (minWinRate > 0) {
        hasActiveGuard = true
      ...
      if (minWorstWindowResolvedSizeShare > 0) {
        hasActiveGuard = true
        if (worstWindowResolvedSizeShare < minWorstWindowResolvedSizeShare) breaches.push(`${prefix} worst sz-cov ${formatPct(worstWindowResolvedSizeShare, 0)}<${formatPct(minWorstWindowResolvedSizeShare, 0)}`)
      }
...
function replaySearchHeadroomSummary(
  ...
  try {
    ...
    const pushMissingMinProofHeadroom = (
      group: ReplaySearchHeadroomCandidate['group'],
      label: string
    ) => {
      headrooms.push({
    ...
    const globalPositiveWindows = replaySearchPositiveWindowCountFromPayload(resultParsed as Record<string, unknown>)
    ...
    const globalWorstActiveWindowAcceptedCount = replaySearchWorstActiveWindowAcceptedCountFromPayload(resultParsed as Record<string, unknown>)
    const globalWorstActiveWindowAcceptedSizeUsd = replaySearchWorstActiveWindowAcceptedSizeUsdFromPayload(resultParsed as Record<string, unknown>)
    ...
    const globalDailyGuardWindowShare = replaySearchDailyGuardWindowShareFromPayload(resultParsed as Record<string, unknown>)
    const globalLiveGuardWindowShare = replaySearchLiveGuardWindowShareFromPayload(resultParsed as Record<string, unknown>)
    const globalDailyGuardRestartWindowShare = replaySearchDailyGuardRestartWindowShareFromPayload(resultParsed as Record<string, unknown>)
    const globalLiveGuardRestartWindowShare = replaySearchLiveGuardRestartWindowShareFromPayload(resultParsed as Record<string, unknown>)
    ...
    if (minResolvedSizeShare > 0) pushHeadroom('global', 'sz-cov', globalResolvedSizeShare, minResolvedSizeShare, replayHeadroomPctPoints, 'min')
    ...
    if (maxDailyGuardWindowShare > 0) pushHeadroom('global', 'd-freq', globalDailyGuardWindowShare, maxDailyGuardWindowShare, replayHeadroomPctPoints, 'max')
    if (maxLiveGuardWindowShare > 0) pushHeadroom('global', 'p-freq', globalLiveGuardWindowShare, maxLiveGuardWindowShare, replayHeadroomPctPoints, 'max')
    if (maxDailyGuardRestartWindowShare > 0) pushHeadroom('global', 'd-rst', globalDailyGuardRestartWindowShare, maxDailyGuardRestartWindowShare, replayHeadroomPctPoints, 'max')
    if (maxLiveGuardRestartWindowShare > 0) pushHeadroom('global', 'p-rst', globalLiveGuardRestartWindowShare, maxLiveGuardRestartWindowShare, replayHeadroomPctPoints, 'max')
    ...
    if (maxAvgWindowEndOpenExposureShare > 0) pushHeadroom('global', 'carry avg', globalAvgWindowEndOpenExposureShare, maxAvgWindowEndOpenExposureShare, replayHeadroomPctPoints, 'max')
    ...
    if (maxCarryRestartWindowShare > 0) pushHeadroom('global', 'carry-rst', globalCarryRestartWindowShare, maxCarryRestartWindowShare, replayHeadroomPctPoints, 'max')
    ...
    if (maxTopTraderSizeShare > 0) pushHeadroom('global', 'wallet sz', topTraderSizeShare, maxTopTraderSizeShare, replayHeadroomPctPoints, 'max')
    ...
    if (maxTopMarketSizeShare > 0) pushHeadroom('global', 'market sz', topMarketSizeShare, maxTopMarketSizeShare, replayHeadroomPctPoints, 'max')
    ...
    if (maxTopEntryPriceBandSizeShare > 0) pushHeadroom('global', 'entry sz', topEntryPriceBandSizeShare, maxTopEntryPriceBandSizeShare, replayHeadroomPctPoints, 'max')
    ...
    if (maxTopTimeToCloseBandSizeShare > 0) pushHeadroom('global', 'horizon sz', topTimeToCloseBandSizeShare, maxTopTimeToCloseBandSizeShare, replayHeadroomPctPoints, 'max')
    ...
    if (minAcceptedWindows > 0) pushHeadroom('global', 'acc-win', globalAcceptedWindows, minAcceptedWindows, replayHeadroomCount, 'min')
    ...
    if (maxTopTwoAcceptingWindowAcceptedShare > 0) pushHeadroom('global', 'top2-acc', globalTopTwoAcceptingWindowAcceptedShare, maxTopTwoAcceptingWindowAcceptedShare, replayHeadroomPctPoints, 'max')
    ...
    if (minWorstWindowPnlUsd > -999_999_999) {
      if (!replaySearchHasProvenWorstWindowPnlFromPayload(resultParsed as Record<string, unknown>)) {
    ...
    for (const [mode, prefix] of [['heuristic', 'heur'], ['xgboost', 'model']] as const) {
      ...
      const activeWindowShare = replaySearchModeActiveWindowAcceptedShareFromPayload(
        payload,
        mode,
        Number(resultParsed.window_count || 0),
        acceptedTotal
      )
      const activeWindowSizeShare = replaySearchModeActiveWindowAcceptedSizeShareFromPayload(
        payload,
        mode,
        Number(resultParsed.window_count || 0),
        acceptedSizeTotal
      ...
      if (minModeResolvedSizeShare > 0) pushHeadroom(mode, `${prefix} sz-cov`, resolvedSizeShare, minModeResolvedSizeShare, replayHeadroomPctPoints, 'min')
      ...
      if (mode === 'heuristic') {
        ...
        if (mixModesEnabled && maxActiveWindowShare > 0) pushHeadroom(mode, `${prefix} acc-mix`, activeWindowShare, maxActiveWindowShare, replayHeadroomPctPoints, 'max')
        const maxActiveWindowSizeShare = Number(constraints.max_heuristic_active_window_accepted_size_share || 0)
        if (mixModesEnabled && maxActiveWindowSizeShare > 0) pushHeadroom(mode, `${prefix} acc-mix$`, activeWindowSizeShare, maxActiveWindowSizeShare, replayHeadroomPctPoints, 'max')
      } else {
        ...
        if (mixModesEnabled && minActiveWindowShare > 0) pushHeadroom(mode, `${prefix} acc-mix`, activeWindowShare, minActiveWindowShare, replayHeadroomPctPoints, 'min')
        const minActiveWindowSizeShare = Number(constraints.min_xgboost_active_window_accepted_size_share || 0)
        if (mixModesEnabled && minActiveWindowSizeShare > 0) pushHeadroom(mode, `${prefix} acc-mix$`, activeWindowSizeShare, minActiveWindowSizeShare, replayHeadroomPctPoints, 'min')
      }
    }
...
function replaySearchWindowSummary(latestSearch: ReplaySearchSummaryRow | null | undefined): string {
  ...
  try {
    ...
    positive = formatCount(replaySearchPositiveWindowCountFromPayload(parsed as Record<string, unknown>))
    negative = formatCount(replaySearchNegativeWindowCountFromPayload(parsed as Record<string, unknown>))
    const windowCount = Number(parsed.window_count || 0)
    ...
    const carryWindowSummary = replaySearchCarryWindowSummaryFromPayload(parsed as Record<string, unknown>)
    const carryRestartSummary = replaySearchCarryRestartSummaryFromPayload(parsed as Record<string, unknown>)
...
export function Models({selectedPanelIndex, detailOpen, selectedSettingIndex, settingsValues}: ModelsProps) {
  ...
  const manualRequestValue = useMemo(() => {
    ...
    if (manualRetrainPending) {
      ...
      if (manualRetrainMessage) retrainParts.push(manualRetrainMessage)
      if (manualRetrainRequestedAt > 0) retrainParts.push(secondsAgo(manualRetrainRequestedAt))
      parts.push(retrainParts.join(' '))
    }
    if (manualTradePending) {
      const tradeParts = ['trade']
      if (manualTradeMessage) tradeParts.push(manualTradeMessage)
      if (manualTradeRequestedAt > 0) tradeParts.push(secondsAgo(manualTradeRequestedAt))
  ...
  const replaySearchValue = useMemo(() => {
    if (replaySearchStatusText === '-' && !replaySearchTriggerText && replaySearchCandidateCount <= 0 && replaySearchFeasibleCount <= 0 && !replaySearchMessageText) return '-'
    const parts: string[] = []
    if (replaySearchStatusText !== '-') parts.push(replaySearchStatusText)
    if (replaySearchTriggerText) parts.push(replaySearchTriggerText)
  ...
  const promotionDeltaValue = useMemo(() => {
    const parts: string[] = []
    if (safePromotionScoreDelta != null) parts.push(`score ${formatPointDelta(safePromotionScoreDelta, 2)}`)
    if (safePromotionPnlDelta != null) parts.push(`pnl ${formatDollar(safePromotionPnlDelta)}`)
    if (!parts.length && promotionRunId > 0) parts.push(`run ${formatCount(promotionRunId)}`)
    if (!parts.length && promotionCandidateId > 0) parts.push(`cand ${formatCount(promotionCandidateId)}`)
    return parts.length ? parts.join(' | ') : '-'
  ...
  const humanModelStatusStats: CompactStatItem[] = [
  ...
  const humanModelArtifactStats: CompactStatItem[] = [
  ...
  const humanEvidenceStats: CompactStatItem[] = [
  ...
  const humanTrainingStats: CompactStatItem[] = [
  ...
  const humanReplayStats: CompactStatItem[] = [
  ...
  const humanConfidenceStats: CompactStatItem[] = [
    {label: 'Resolved', value: formatCount(calibration?.resolved)},
  ...
  const humanSignalModeStats: CompactStatItem[] = signalModes
    .filter((row) => row.mode.trim().toLowerCase() !== 'veto')
    .slice()
  ...
  const humanExitStats: CompactStatItem[] = [
  ...
  const renderModelRowsLeft = (items: CompactStatItem[], width: number, labelWidth = modelRowLabelWidth(width)) =>
    items.map((item) => (
      <DenseModelsRow
        key={item.label}
        label={item.label}
    ...
    <InkBox width="100%" flexDirection="column">
      ...
      <InkBox width="100%">
        <InkBox width={modelsColumnWidths[0]} flexDirection="column">
          ...
          <ModelsSpacer />
          <ModelsSubsectionTitle title="Artifact Quality" width={modelsColumnWidths[0]} />
        ...
        <InkBox width={modelsColumnWidths[1]} flexDirection="column">
          ...
          {latestSharedHoldoutRun && latestSharedHoldout ? (
            <>
              <ModelsSpacer />
              <ModelsSubsectionTitle title="Holdout Gate" width={modelsColumnWidths[1]} />
              {renderModelRows(
                [
                  {
                    label: 'Result',
                ...
                modelsColumnWidths[1]
              )}
            </>
          ) : null}
          <ModelsSpacer />
          <ModelsSubsectionTitle title="Recent Training Runs" width={modelsColumnWidths[1]} />
          {compactRecentRetrainRows.length ? (
            renderModelRowsLeft(compactRecentRetrainRows, modelsColumnWidths[1], 8)
          ) : (
            <Text color={theme.dim}>{fit('No retrain attempts logged yet.', modelsColumnWidths[1])}</Text>
          )}
        </InkBox>
        ...
        <InkBox width={modelsColumnWidths[3]} flexDirection="column">
          <ModelsSectionTitle
            ...
            backgroundColor={selectedRowBackground}
          />
          {humanSignalModeStats.length ? (
            renderModelRowsLeft(humanSignalModeStats, modelsColumnWidths[3], 10)
          ) : (
