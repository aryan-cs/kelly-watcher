...
export const MODEL_PANEL_DEFS: ModelPanelDefinition[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      {label: 'Score weights', text: 'Active replay-search score weights on the latest search run, including drawdown, instability, worst-window loss, peak open-exposure, window-end carry exposure, carry-window frequency, carry-restart continuity, count and deployed-dollar accepting-window depth, daily/live guard frequency and restart continuity, count-weighted and deployed-dollar coverage, global and worst-window count-weighted and deployed-dollar coverage, scorer-path count-weighted and deployed-dollar coverage and depth, scorer accepting-window mix, and count/share plus deployed-dollar concentration terms.'},
      {label: 'Best score', text: 'Best feasible score decomposition: replay P&L minus drawdown, instability, worst-window loss, peak open-exposure, window-end carry exposure, carry-window frequency, carry-restart continuity, count and deployed-dollar accepting-window depth, daily/live guard frequency and restart continuity, count-weighted and deployed-dollar coverage, worst-window count-weighted and deployed-dollar coverage, scorer-path count-weighted and deployed-dollar coverage and depth, scorer-loss, scorer-inactivity, and concentration penalties.'},
      {label: 'Search robust', text: 'Best feasible search candidate P&L and drawdown.'},
      {label: 'Search windows', text: 'Positive versus negative windows, active versus idle participation, carry-window frequency, carry-restart continuity risk, sparsest accepting-window trade depth and deployed dollars, peak and average window-end carry exposure, and the worst window P&L for the latest best feasible search candidate.'},
      ...
      {label: 'Mode pen', text: 'Soft scorer-path ranking weights from the latest replay search, for scorer coverage, scorer deployed-dollar coverage, scorer worst-window count-weighted coverage, scorer worst-window deployed-dollar coverage, scorer active-window count depth, scorer active-window deployed-dollar depth, scorer active-window mix, scorer-loss, and scorer-inactivity pressure.'},
      ...
      {label: 'Cur score', text: 'Current/base score decomposition: replay P&L minus drawdown, instability, worst-window loss, peak open-exposure, window-end carry exposure, carry-window frequency, carry-restart continuity, daily/live guard frequency and restart continuity, count-weighted and deployed-dollar coverage, worst-window count-weighted and deployed-dollar coverage, global window inactivity, scorer-path count-weighted and deployed-dollar coverage and depth, scorer-loss, scorer-inactivity, and concentration penalties.'},
      {label: 'Score drift', text: 'Best feasible minus current/base score decomposition, split into replay P&L and each score penalty term, including carry-window frequency, carry-restart continuity, daily/live guard frequency and restart continuity, count-weighted and deployed-dollar coverage, worst-window count-weighted and deployed-dollar coverage, inactivity, scorer-path count-weighted and deployed-dollar coverage/depth, and concentration penalties.'},
...
function replaySearchModeMixSummary(
  ...
  try {
    ...
    const parsedEntries = Object.entries(rawSummary as Record<string, unknown>)
      ...
      .filter((entry): entry is {mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number} => Boolean(entry))
      .filter((entry) => {
    ...
    const entries: Array<{mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number}> = []
...
function replaySearchCurrentModeEvidenceSummary(
  ...
  try {
    ...
    const parsedEntries = Object.entries(rawSummary as Record<string, unknown>)
      ...
      .filter((entry): entry is {mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number; winRate: number | null} => Boolean(entry))
      .filter((entry) => {
        if (entry.mode === 'heuristic') return enabled.heuristic
        if (entry.mode === 'xgboost') return enabled.xgboost
        return true
    ...
    const entryByMode = new Map(parsedEntries.map((entry) => [entry.mode, entry]))
    const entries: Array<{mode: string; acceptedCount: number; resolvedCount: number; acceptedSizeUsd: number; resolvedSizeUsd: number; totalPnlUsd: number; winRate: number | null}> = []
    if (enabled.heuristic) {
      entries.push(entryByMode.get('heuristic') ?? {
        mode: 'heuristic',
        acceptedCount: 0,
...
function replaySearchModeFloorSummary(
  ...
  try {
    ...
    if (!enabled.heuristic) {
      ...
      if (mixModesEnabled && maxHeuristicActiveWindowAcceptedShare > 0) parts.push(`heur win-mix<=${formatPct(maxHeuristicActiveWindowAcceptedShare, 0)}`)
      if (mixModesEnabled && maxHeuristicActiveWindowAcceptedSizeShare > 0) parts.push(`heur win-mix$<=${formatPct(maxHeuristicActiveWindowAcceptedSizeShare, 0)}`)
    }
    if (!enabled.xgboost) {
      ...
      if (mixModesEnabled && minXgboostActiveWindowAcceptedShare > 0) parts.push(`model win-mix>=${formatPct(minXgboostActiveWindowAcceptedShare, 0)}`)
      if (mixModesEnabled && minXgboostActiveWindowAcceptedSizeShare > 0) parts.push(`model win-mix$>=${formatPct(minXgboostActiveWindowAcceptedSizeShare, 0)}`)
    }
...
function replaySearchScoreBreakdownSummary(raw: string | null | undefined): string {
  ...
  try {
    ...
    if (Math.abs(modeActiveWindowAcceptedSharePenaltyUsd) > 1e-9) parts.push(`m-win-mix ${formatDollar(-modeActiveWindowAcceptedSharePenaltyUsd)}`)
    if (Math.abs(modeActiveWindowAcceptedSizeSharePenaltyUsd) > 1e-9) parts.push(`m-win-mix$ ${formatDollar(-modeActiveWindowAcceptedSizeSharePenaltyUsd)}`)
...
function replaySearchScoreDriftSummary(
  ...
  if (Math.abs(modeActiveWindowMixDelta) > 1e-9) parts.push(`m-win-mix ${formatDollar(modeActiveWindowMixDelta)}`)
  if (Math.abs(modeActiveWindowSizeMixDelta) > 1e-9) parts.push(`m-win-mix$ ${formatDollar(modeActiveWindowSizeMixDelta)}`)
...
function replaySearchModePenaltySummary(row: ReplaySearchSummaryRow | undefined): string {
  ...
  if (modeActiveWindowAcceptedSharePenalty > 0) parts.push(`win-mix ${modeActiveWindowAcceptedSharePenalty.toFixed(2)}x`)
  if (modeActiveWindowAcceptedSizeSharePenalty > 0) parts.push(`win-mix$ ${modeActiveWindowAcceptedSizeSharePenalty.toFixed(2)}x`)
  ...
  try {
    ...
    for (const [mode, prefix] of [['heuristic', 'heur'], ['xgboost', 'model']] as const) {
      ...
      if (mode === 'heuristic') {
        ...
        if (mixModesEnabled && maxActiveWindowShare > 0) pushHeadroom(mode, `${prefix} win-mix`, activeWindowShare, maxActiveWindowShare, replayHeadroomPctPoints, 'max')
        const maxActiveWindowSizeShare = Number(constraints.max_heuristic_active_window_accepted_size_share || 0)
        if (mixModesEnabled && maxActiveWindowSizeShare > 0) pushHeadroom(mode, `${prefix} win-mix$`, activeWindowSizeShare, maxActiveWindowSizeShare, replayHeadroomPctPoints, 'max')
      } else {
        ...
        if (mixModesEnabled && minActiveWindowShare > 0) pushHeadroom(mode, `${prefix} win-mix`, activeWindowShare, minActiveWindowShare, replayHeadroomPctPoints, 'min')
        const minActiveWindowSizeShare = Number(constraints.min_xgboost_active_window_accepted_size_share || 0)
        if (mixModesEnabled && minActiveWindowSizeShare > 0) pushHeadroom(mode, `${prefix} win-mix$`, activeWindowSizeShare, minActiveWindowSizeShare, replayHeadroomPctPoints, 'min')
      }
    }
...
function replaySearchWindowSummary(latestSearch: ReplaySearchSummaryRow | null | undefined): string {
  ...
  try {
    ...
    if (windowCount <= 1) return `${positive}+ / ${negative}-${carrySuffix}${carryFreqSuffix}${carryRestartSuffix} | ${worst}`
