...
`uv` manages Python dependencies only. The terminal dashboard is a separate Node.js application that lives in the `dashboard-cli/` subfolder with its own `package.json`. Install its dependencies once:
...
The `dashboard-cli/package.json` (create this manually in the `dashboard-cli/` folder):
...
|
+-- dashboard-cli/               # Terminal dashboard (separate Node.js app)
...
The `dashboard-cli/` folder is a completely independent Node application. It reads from `data/trading.db` and `data/events.jsonl` — both written by the Python bot — but never imports Python code and runs in a separate terminal process. You can kill and restart the dashboard at any time without affecting the bot.

### `dashboard-cli/tsconfig.json`
...
The dashboard is a Node.js app written in TypeScript. It lives in `dashboard-cli/` and has no Python dependency. Install once with `npm install` inside that folder (see Section 20 quick-start).
...
The `dashboard-cli/` folder structure is shown in full in Section 20. The entry point is `dashboard-cli/dashboard.tsx`.

Run with:

```bash
cd dashboard-cli && npm start
```
...
```typescript
// dashboard-cli/useEventStream.ts
...
#### `dashboard-cli/theme.ts` — single source of truth for all colors

```typescript
// dashboard-cli/theme.ts
...
```typescript
// dashboard-cli/components/Box.tsx
...
```typescript
// dashboard-cli/components/BarSparkline.tsx
import React from 'react'
...
#### `dashboard-cli/dashboard.tsx` — root app

```typescript
// dashboard-cli/dashboard.tsx
...
`npm start` runs `tsx dashboard.tsx` as defined in `dashboard-cli/package.json`. The dashboard starts immediately; it will show empty tables until the bot begins writing events and DB rows.
