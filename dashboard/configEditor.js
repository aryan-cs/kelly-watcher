...
export const logLevelPresets = ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'];
...
export function validateEditableConfigValue(field, raw) {
    ...
    const isCostField = field.key === 'ENTRY_FIXED_COST_USD' ||
    ...
    if ((field.key === 'WARM_POLL_INTERVAL_MULTIPLIER' ||
    ...
    if ((field.key === 'WALLET_DISCOVERY_SIZE_MULTIPLIER' || field.key === 'WALLET_PROBATION_SIZE_MULTIPLIER') &&
        (numeric <= 0 || numeric > 1)) {
...
export function formatEditableConfigValue(field, value) {
    ...
    if (field.key === 'SHADOW_BANKROLL_USD' ||
