...
export function LiveFeed({ scrollOffset = 0, onScrollOffsetChange }) {
    ...
    return (React.createElement(Box, { height: "100%" },
        ...
        React.createElement(InkBox, { flexDirection: "column", marginTop: 1 }, events.length ? (events.map((event, index) => (React.createElement(TradeRow, { key: `incoming-${effectiveOffset + index}-${event.trade_id}-${event.ts}`, layout: "feed", maxWidth: tableWidth, displayId: tradeIdLookup.get(event.trade_id), ts: event.ts, username: event.username, trader: event.trader, question: event.question, marketUrl: event.market_url, side: event.side, action: event.action, price: event.price, shares: event.shares ?? event.size_usd, sizeUsd: event.amount_usd ?? event.size_usd * event.price })))) : (React.createElement(Text, { color: theme.dim }, "Waiting for incoming trade events..."))),
