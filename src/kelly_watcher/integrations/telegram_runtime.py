...
from kelly_watcher.config import dashboard_api_port, dashboard_web_url, telegram_bot_token, telegram_chat_id
...
def _tailscale_magicdns_name() -> str:
    for env_name in ("TAILSCALE_DNS_NAME", "TS_DNS_NAME"):
    ...
    host_name = str(self_payload.get("HostName") or "").strip().rstrip(".")
    magicdns_suffix = str(payload.get("MagicDNSSuffix") or "").strip().rstrip(".")
...
def _dashboard_link_message() -> str:
    url = dashboard_web_url()
    if not url:
        url = _tailscale_dashboard_url()
    if not url:
        return (
