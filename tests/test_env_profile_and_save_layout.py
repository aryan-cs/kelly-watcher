...
class EnvProfileTests(unittest.TestCase):
    def test_env_path_for_profile_points_to_single_repo_env_file(self) -> None:
    ...
    def test_active_env_path_is_repo_dot_env_even_if_legacy_save_env_exists(self) -> None:
        with TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
    ...
    def test_ensure_persistent_env_path_migrates_legacy_save_env_into_repo_root(self) -> None:
        with TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            save_env = repo_root / "save" / ".env"
            save_env.parent.mkdir(parents=True, exist_ok=True)
            ...
            repo_env = env_profile.ensure_persistent_env_path("default", repo_root=repo_root)

            self.assertEqual(repo_env, repo_root / ".env")
            self.assertEqual(repo_env.read_text(encoding="utf-8"), "TELEGRAM_BOT_TOKEN=x\n")
