...
function blendRgb(red, green, blue, scrimRgb, strength) {
    return [
...
function ansiIndexedColorToRgb(index) {
    if (index < 16) {
    ...
    const blue = value % 6;
    const cubeToRgb = (channel) => (channel === 0 ? 0 : 55 + (channel * 40));
    return [cubeToRgb(red), cubeToRgb(green), cubeToRgb(blue)];
}
function ansiNamedColorToRgb(code) {
    if (code >= 30 && code <= 37) {
...
function transformAnsiSequence(code, scrimRgb) {
    if (!code.trim()) {
        return `\u001b[${code}m`;
    }
    ...
    for (let index = 0; index < params.length; index += 1) {
        ...
        if ((param === 38 || param === 48) && params[index + 1] === '2' && index + 4 < params.length) {
            ...
            if (Number.isFinite(red) && Number.isFinite(green) && Number.isFinite(blue)) {
                const [nextRed, nextGreen, nextBlue] = blendRgb(red, green, blue, scrimRgb, param === 38 ? 0.42 : 0.32);
        ...
        if (namedColor) {
            const [red, green, blue] = blendRgb(namedColor[0], namedColor[1], namedColor[2], scrimRgb, param >= 40 && param <= 107 ? 0.32 : 0.42);
...
function applyOverlayLine(line, backgroundCode, scrimRgb) {
    const recolored = line.replace(ANSI_SGR_PATTERN, (_, code) => transformAnsiSequence(code, scrimRgb));
...
export function ModalOverlay({ children, backdrop, backgroundColor }) {
    ...
    return (React.createElement(InkBox, { position: "absolute", width: "100%", height: "100%" },
        backdrop ? (React.createElement(InkBox, { position: "absolute", width: "100%", height: "100%" },
            React.createElement(Transform, { transform: (line) => applyOverlayLine(line, backgroundCode, scrimRgb) }, backdrop))) : null,
