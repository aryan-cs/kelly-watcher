...
def cleanup_premature_resolutions(backup_path: Path | None = None) -> dict[str, int | str]:
    runtime_db_path = db.DB_PATH
    if not runtime_db_path.exists():
        raise FileNotFoundError(f"Database not found at {runtime_db_path}")

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    backup_target = backup_path or runtime_db_path.with_name(
        f"{runtime_db_path.stem}.premature_cleanup_{timestamp}.bak"
    )
    backup_target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(runtime_db_path, backup_target)
