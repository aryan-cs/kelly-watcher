...
export const MODEL_PANEL_DEFS = [
    ...
    },
    {
        ...
        rows: [
            ...
            { label: 'Score weights', text: 'Active replay-search score weights on the latest search run, including drawdown, instability, worst-window loss, peak open-exposure, window-end carry exposure, carry-window frequency, carry-restart continuity, count and deployed-dollar active-window depth, pause-guard, global coverage, global window inactivity, worst-window coverage, scorer coverage, scorer worst-window coverage, scorer-window depth, scorer active-window mix, scorer-loss, scorer-inactivity, and concentration terms.' },
            { label: 'Best score', text: 'Best feasible score decomposition: replay P&L minus drawdown, instability, worst-window loss, peak open-exposure, window-end carry exposure, carry-window frequency, carry-restart continuity, active-window depth, pause-guard, global coverage, global window inactivity, worst-window coverage, scorer coverage, scorer worst-window coverage, scorer-window depth, scorer-loss, scorer-inactivity, and concentration penalties.' },
            ...
            { label: 'Cur score', text: 'Current/base score decomposition: replay P&L minus drawdown, instability, worst-window loss, peak open-exposure, window-end carry exposure, carry-window frequency, carry-restart continuity, pause-guard, global coverage, global window inactivity, worst-window coverage, scorer coverage, scorer worst-window coverage, scorer-loss, scorer-inactivity, and concentration penalties.' },
...
function replaySearchPauseGuardSummary(bestRaw, currentRaw, constraintsRaw, pauseGuardPenalty, liveGuardWindowPenalty) {
...
function replaySearchHeadroomSummary(resultRaw, constraintsRaw, policyRaw) {
    ...
    try {
        ...
        if (maxLiveGuardWindowShare > 0)
            pushHeadroom('global', 'pause-freq', globalLiveGuardWindowShare, maxLiveGuardWindowShare, replayHeadroomPctPoints, 'max');
...
export function Models({ selectedPanelIndex, detailOpen, selectedSettingIndex, settingsValues }) {
    ...
    const replaySearchPauseGuard = useMemo(() => replaySearchPauseGuardSummary(latestReplaySearch?.result_json, latestReplaySearch?.current_candidate_result_json, latestReplaySearch?.constraints_json, latestReplaySearch?.pause_guard_penalty, latestReplaySearch?.live_guard_window_penalty), [
