...
export function Performance({
  ...
  const performanceQueryBlockedMessage =
  ...
  const renderPageBody = () => (
    <>
      {performanceQueryBlockedMessage ? (
        <InkBox marginBottom={1}>
          <Text color={theme.yellow}>{fit(performanceQueryBlockedMessage, positionsTableWidth)}</Text>
        </InkBox>
      ) : null}
      <InkBox flexDirection={stacked ? 'column' : 'row'}>
        ...
        <Box title={`Hourly ${activeTitle} P&L`} width={stacked ? '100%' : '50%'} accent={selectedBox === 'daily'}>
          {dailyPreviewEntries.length ? (
            ...
            <Text color={performanceQueryBlockedMessage ? theme.yellow : theme.dim}>
              {fit(
                performanceQueryBlockedMessage || `No resolved ${activeTitle.toLowerCase()} trades yet.`,
        ...
        <InkBox height={paneMetrics.paneHeight}>
          ...
          >
            {visibleCurrentPositions.length ? (
              ...
              <Text color={performanceQueryBlockedMessage ? theme.yellow : theme.dim}>
                {fit(performanceQueryBlockedMessage || 'No open positions right now.', positionsTableWidth)}
              </Text>
            )}
          </Box>
        </InkBox>
        ...
        <InkBox height={paneMetrics.paneHeight}>
          ...
          >
            {visiblePastPositions.length ? (
              ...
              <Text color={performanceQueryBlockedMessage ? theme.yellow : theme.dim}>
                {fit(performanceQueryBlockedMessage || 'No past positions yet.', positionsTableWidth)}
              </Text>
            )}
          </Box>
        </InkBox>
