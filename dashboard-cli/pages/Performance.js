...
export function Performance({ currentScrollOffset, pastScrollOffset, activePane, selectedBox, dailyDetailOpen, dailyDetailScrollOffset, actionState, editState, pendingPerfExits = [], onCurrentScrollOffsetChange, onPastScrollOffsetChange, onDailyDetailScrollOffsetChange, onSelectionMetaChange, onDetailHistoryMetaChange, onPendingPerfExitSettlement }) {
    ...
    const performanceQueryBlockedMessage = shadowRestartPending
        ? `Performance queries are blocked: ${shadowRestartMessage}`
        : startupRecoveryOnly
            ? `Performance queries are blocked: ${startupBlockReason || startupDetail || 'startup blocked in recovery-only mode'}`
            : dbIntegrityKnown && !dbIntegrityOk
                ? `Performance queries are blocked: ${dbIntegrityMessage || 'SQLite integrity check failed.'}`
                : '';
    ...
    const renderPageBody = () => (React.createElement(React.Fragment, null,
        performanceQueryBlockedMessage ? (React.createElement(InkBox, { marginBottom: 1 },
            React.createElement(Text, { color: theme.yellow }, fit(performanceQueryBlockedMessage, positionsTableWidth)))) : null,
        ...
        React.createElement(InkBox, { marginTop: 1, flexDirection: "column", height: paneMetrics.paneHeight * 2 + 1 },
            React.createElement(InkBox, { height: paneMetrics.paneHeight },
                ...
                })) : (React.createElement(Text, { color: performanceQueryBlockedMessage ? theme.yellow : theme.dim }, fit(performanceQueryBlockedMessage || 'No open positions right now.', positionsTableWidth))))),
            ...
            React.createElement(InkBox, { height: paneMetrics.paneHeight },
                ...
                })) : (React.createElement(Text, { color: performanceQueryBlockedMessage ? theme.yellow : theme.dim }, fit(performanceQueryBlockedMessage || 'No past positions yet.', positionsTableWidth))))))));
    return (React.createElement(InkBox, { flexDirection: "column", width: "100%" },
        renderPageBody(),
