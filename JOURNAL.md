...
---
[2026-04-24 12:41 CDT] codex-main
Task: Analyze local `save/` data, split env files into safe config and private secrets, and tune shadow-mode settings from observed history.
Claims: `JOURNAL.md`, `.gitignore`, `config.env.example`, `secrets.env.example`, `env_profile.py`, `config.py`, `dashboard_api.py`, `shadow_reset.py`, `dashboard-cli/envProfile.*`, `dashboard-cli/paths.*`, `dashboard-cli/api.*`, `README.md`, `tests/test_env_profile_and_save_layout.py`, local ignored `config.env`, local ignored `secrets.env`
Status: Completed locally; pending commit and push.
Blockers: The live `save/data/trading.db` on this Mac checkout is structurally unhealthy: immutable read-only `PRAGMA quick_check` reported malformed btrees and aggregate scans of `trade_log` failed with `database disk image is malformed`. I therefore based tuning on the latest verified clean backup `save/data/db_backups/trading.20260423_040135.db.gz`.
Next: Push `master`; operator should copy the generated ignored `config.env` / `secrets.env` to the Windows backend checkout or recreate them from the new examples, then recover/replace the malformed live DB before trusting live-mode readiness.
Decisions: New env loading reads `config.env` plus `secrets.env`; legacy `.env` remains a fallback only when the split files are absent. Dashboard/API env editors write safe values to `config.env`; private tokens/URLs stay in `secrets.env`. The generated local `config.env` keeps shadow mode, disables heuristic scoring, keeps XGBoost enabled, sets `MIN_CONFIDENCE=0.60`, `XGBOOST_ALLOWED_ENTRY_PRICE_BANDS=0.50-0.54,0.60-0.69,>=0.70`, `MAX_MARKET_HORIZON=6h`, `MAX_SOURCE_TRADE_AGE=45s`, `MAX_ORDERBOOK_STALENESS=2s`, `MAX_BET_FRACTION=0.04`, total exposure cap `0.25`, and market/trader caps `0.05`. I also replaced the old 203-wallet env watchlist with the 26 active, syntactically valid wallets from the clean backup's active registry.
Analysis: Clean backup summary was 106,121 logged decisions, 801 accepted, 795 resolved, backup accepted-history PnL -$214.81 on $23,885.30 staked (PF 0.96, max drawdown about $984). By mode, XGBoost was +$200.40 while heuristic was -$625.97. XGBoost with <=6h horizon, <=45s source latency, confidence >=0.60, and entry bands `0.50-0.54,0.60-0.69,>=0.70` selected 127 resolved historical fills with +$931.30 PnL, 87.4% win rate, PF 4.24, about 21.4% historical stake ROI, and about $120.91 max PnL drawdown. This is historical/shadow evidence only, not a profit guarantee.
Tests: `UV_CACHE_DIR=/tmp/uv-cache-kelly uv run python -m py_compile env_profile.py config.py dashboard_api.py shadow_reset.py main.py tests/test_env_profile_and_save_layout.py tests/test_shadow_reset.py` -> passed; `UV_CACHE_DIR=/tmp/uv-cache-kelly uv run pytest tests/test_env_profile_and_save_layout.py tests/test_cli.py tests/test_shadow_reset.py -q` -> 19 passed; `UV_CACHE_DIR=/tmp/uv-cache-kelly uv run pytest tests/test_runtime_fixes.py -q -k "config_value_response or max_daily_loss_pct_reloads_from_env_file or runtime_env or shadow_reset or restart or env"` -> 29 passed, 184 deselected; `cd dashboard-cli && npm exec tsc -- --noEmit` -> passed; split-env smoke check loaded 26 wallets, heuristic disabled, XGBoost config active, and Telegram token present without printing it; `git diff --check` -> passed.
...
Status: Completed locally; pending commit and push.
Blockers: None.
Next: Windows should pull and run `start-backend-windows.cmd`; Mac should pull and run `./start-dashboard-mac.sh` after `dashboard-cli/npm install`.
...
Claims: `JOURNAL.md`, rollback tree, `env_profile.py`, `dashboard-cli/envProfile.*`, `dashboard-cli/paths.*`, `dashboard-cli/package.json`, `kelly_watcher/cli.py`, `kelly_watcher/shadow_reset.py`, `README.md`, env/profile tests
...
Next: After pull on Windows, recreate/fill the single `.env`, run `uv sync`, run `cd dashboard-cli && npm install` if needed, then start backend with `uv run main` and terminal dashboard with `cd dashboard-cli && npm start`.
Decisions: Restored the old root-level Python + Ink dashboard tree, removed `dashboard-web` and the `src/kelly_watcher` package layout, kept the terminal dashboard under `dashboard-cli/`, and simplified env loading so only `.env` is read/written. Old `--dev`/`--prod` flags are accepted as no-ops where argparse still sees them, but they no longer select profile-specific files. Excluded the old commit's non-runtime macOS alias file and empty `x` file from the rollback.
...
Next: Use `cd dashboard-cli-web && npm run dev` locally for design iteration; do not push to Windows until the UI shell is approved and ready to land as one batch.
...
[2026-04-24 13:17 CDT] codex-main
Task: Clean up rollback repository layout and make `config.env` safe to track while keeping `secrets.env` private.
Claims: `JOURNAL.md`, `.gitignore`, `config.env`, `pyproject.toml`, `README.md`, `main.py`, `restart_shadow.py`, `src/kelly_watcher/**`, `dashboard-cli/**`, `docs/PLAN.md`, `tests/**`
Status: Completed
Blockers: None. `secrets.env` remains ignored and untracked; production still needs local secret values on the Windows backend and Mac CLI host.
Next: Pull `master` on the Windows backend and Mac frontend. Keep editing non-secret runtime settings in `config.env`; put Telegram, private key, wallet address, and backend/API addresses only in `secrets.env`.
Decisions: Restored the organized Python package layout under `src/kelly_watcher/` instead of the flattened root-module layout introduced during rollback. Restored `dashboard-cli/` and `docs/PLAN.md`, kept root wrappers for `main.py` and `restart_shadow.py`, and updated package entry points/imports/tests to the package paths. Added pytest isolation so the committed operator `config.env` does not leak tuned production settings into unit tests. Audited `config.env` for secret-like keys before tracking it; no token/private/password/chat/API-key fields were present.
Tests: `UV_CACHE_DIR=/tmp/uv-cache-kelly uv run python -m py_compile main.py restart_shadow.py $(find src/kelly_watcher -name '*.py' -print)` -> passed; `UV_CACHE_DIR=/tmp/uv-cache-kelly uv run pytest -q` -> 740 passed, 2 skipped, 45 subtests passed; `npm exec tsc -- --noEmit` in `dashboard-cli/` -> passed.

[2026-04-24 14:55 CDT] codex-main
Task: Resolve cleanup rebase on top of current `master`.
Claims: `JOURNAL.md`, `.gitignore`, `README.md`, `dashboard-cli/**`, `src/kelly_watcher/env_profile.py`, `src/kelly_watcher/config.py`, `src/kelly_watcher/dashboard_api.py`, `src/kelly_watcher/integrations/telegram_runtime.py`, `src/kelly_watcher/shadow_reset.py`, selected tests, deleted root helper scripts and `.env.example`.
Status: Completed
Blockers: None in the worktree; rebase continuation/push depends on git metadata writes succeeding from this checkout.
Next: Continue the rebase and push `master`. Keep `config.env` tracked, keep `secrets.env` ignored, and do not reintroduce `.sh`, `.cmd`, or `.bat` helper files at repo root.
Decisions: Kept remote `master`'s rollback/engine files as the base, then reapplied the repository cleanup only where it still belongs. Removed legacy `.env.example` and legacy env-file fallback paths so config is root `config.env` plus root `secrets.env`. Preserved `--local` process overrides for Mac same-device testing and Telegram `/link` support. Deleted root helper scripts (`restart_shadow.sh`, `start-backend-windows.cmd`, `start-dashboard-mac.sh`, `watchdog.bat`) instead of keeping multiple launch paths.
Tests: `UV_CACHE_DIR=/tmp/uv-cache-kelly uv run python -m py_compile $(find src/kelly_watcher -name '*.py' -print)` -> passed; `npm exec tsc -- --noEmit` in `dashboard-cli` -> passed; focused env/Telegram/replay-search tests -> 10 passed; secret-like scan found no populated secret values in `config.env`, `config.env.example`, or `secrets.env.example`.
