...
export function Signals({ scrollOffset = 0, horizontalOffset = 0 }) {
    ...
    return (React.createElement(Box, { height: "100%" },
        ...
        React.createElement(InkBox, { flexDirection: "column", marginTop: 1 }, signals.length ? (signals.map((event) => (React.createElement(TradeRow, { key: `${event.trade_id}-${event.ts}`, layout: "signals", maxWidth: visibleWidth, viewportOffset: effectiveHorizontalOffset, displayId: tradeIdLookup.get(event.trade_id), ts: event.ts, username: event.username, trader: event.trader, question: event.question, side: event.side, action: event.action ?? incomingActionByTradeId.get(event.trade_id), price: event.price, shares: event.shares ?? (event.price > 0 ? event.size_usd / event.price : 0), sizeUsd: event.amount_usd ?? event.size_usd, decision: event.decision, confidence: event.confidence, reason: event.reason })))) : (React.createElement(Text, { color: theme.dim }, "No scored signals yet."))),
