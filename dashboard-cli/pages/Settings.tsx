...
export function Settings({editor}: SettingsProps) {
  ...
  const databaseStats: SummaryStat[] = [
    {label: 'Rows', value: settingsRowsValue, color: settingsRowsColor},
  ...
  return (
    <InkBox flexDirection="column" width="100%">
      ...
      <InkBox marginTop={1}>
        <Box title="Live Readiness" width="100%" accent={liveModeReady}>
          <InkBox flexDirection="column" marginTop={1}>
            <StatRow
              label="Live trading"
              value={liveTradingEnabled ? 'enabled in config' : 'disabled in config'}
              color={liveTradingEnabled ? theme.green : theme.red}
            />
            <StatRow label="DB integrity" value={dbIntegrityStatus} color={dbIntegrityColor} />
            <StatRow label="All-time shadow history" value={shadowHistoryStateKnown ? shadowGateStatus : 'checking'} color={shadowHistoryStateKnown ? shadowGateColor : theme.dim} />
            <StatRow label="Segment shadow" value={shadowSegmentStatus} color={shadowSegmentColor} />
            <StatRow label="Overall" value={liveReadinessStatus} color={liveReadinessColor} />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Details</Text>
              {wrapText(liveReadinessSummary, Math.max(24, panelContentWidth)).map((line, index) => (
                <Text key={`live-readiness-summary-${index}`} color={liveReadinessColor}>
                  {line}
                </Text>
              ))}
              {shadowReadinessSummaryLines.map((line, index) => (
                <Text key={`shadow-readiness-summary-${index}`} color={shadowGateColor}>
                  {line}
                </Text>
              ))}
              {shadowReadinessReasonLines.map((line, index) => (
                <Text key={`shadow-readiness-reason-${index}`} color={shadowGateColor}>
                  {line}
                </Text>
              ))}
            </InkBox>
      ...
      <InkBox marginTop={1}>
        <Box title="Shadow History Epoch" width="100%" accent={shadowHistoryEpochKnown && shadowHistoryEpochStartedAt > 0}>
          <InkBox flexDirection="column" marginTop={1}>
            <Text color={theme.dim}>Shadow-history evidence epoch only. Source and active scope labels describe the evidence window, not the live runtime state.</Text>
            <StatRow label="Status" value={shadowHistoryEpochStatus} color={shadowHistoryEpochColor} />
            <StatRow label="Started" value={shadowHistoryEpochStartedAt > 0 ? formatSettingsDateTime(shadowHistoryEpochStartedAt) : '-'} color={theme.white} />
            <StatRow label="Source" value={shadowHistoryEpochSourceLabel || '-'} color={theme.white} />
            <StatRow label="Active scope" value={shadowHistoryEpochActiveScopeLabel || '-'} color={theme.white} />
            <StatRow label="Current window" value={formatNumber(shadowHistoryEpochReadyCount)} color={theme.white} />
            <StatRow label="Blocked" value={formatNumber(shadowHistoryEpochBlockedCount)} color={shadowHistoryEpochBlockedCount > 0 ? theme.red : theme.dim} />
            <StatRow label="Total resolved" value={formatNumber(shadowHistoryEpochTotalResolved)} color={theme.white} />
            <StatRow label="Legacy/all-time" value={formatNumber(shadowHistoryEpochLegacyResolved)} color={theme.white} />
            <StatRow label="Routed resolved" value={formatNumber(shadowHistoryEpochRoutedResolved)} color={theme.white} />
            <StatRow label="Coverage" value={shadowHistoryEpochCoverageLabel} color={shadowHistoryEpochCoveragePct == null ? theme.dim : theme.white} />
            <StatRow label="Gate" value={shadowHistoryEpochGateSufficient ? 'sufficient' : 'insufficient'} color={shadowHistoryEpochGateSufficient ? theme.green : theme.yellow} />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Summary</Text>
              {shadowHistoryEpochSummaryLines.map((line, index) => (
                <Text key={`shadow-history-epoch-summary-${index}`} color={shadowHistoryEpochColor}>
                  {line}
                </Text>
              ))}
              {shadowHistoryEpochDetailWrappedLines.length > 0 ? (
                <InkBox flexDirection="column" marginTop={1}>
                  <Text color={theme.dim}>Warnings</Text>
                  {shadowHistoryEpochDetailWrappedLines.map((line, index) => (
                    <Text key={`shadow-history-epoch-detail-${index}`} color={shadowHistoryEpochColor}>
                      {line}
                    </Text>
                  ))}
                </InkBox>
      ...
      <InkBox marginTop={1}>
        <Box title="Segment Shadow Readiness" width="100%" accent={shadowSegmentStateKnown && shadowSegmentBlockedCount === 0}>
          <InkBox flexDirection="column" marginTop={1}>
            <StatRow label="Status" value={shadowSegmentStatus} color={shadowSegmentColor} />
            <StatRow label="Scope" value={shadowSegmentScopeLabel} color={theme.dim} />
            <StatRow label="Ready" value={`${formatNumber(shadowSegmentReadyCount)}/${formatNumber(shadowSegmentTotal)}`} color={shadowSegmentColor} />
            <StatRow label="Min resolved" value={shadowSegmentMinResolved > 0 ? formatNumber(shadowSegmentMinResolved) : '-'} color={theme.dim} />
            <StatRow label="Positive" value={formatNumber(shadowSegmentPositiveCount)} color={theme.green} />
            <StatRow label="Negative" value={formatNumber(shadowSegmentNegativeCount)} color={theme.red} />
            <StatRow label="Blocked" value={formatNumber(shadowSegmentBlockedCount)} color={shadowSegmentBlockedCount > 0 ? theme.red : theme.dim} />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Checks include pnl, expectancy, profit factor, calibration gap, and fill-cost slippage.</Text>
              <Text color={theme.dim}>Summary</Text>
              {shadowSegmentSummaryLines.map((line, index) => (
                <Text key={`shadow-segment-summary-${index}`} color={shadowSegmentColor}>
                  {line}
                </Text>
              ))}
              {shadowSegmentSummaryJsonLines.map((line, index) => (
                <Text key={`shadow-segment-summary-json-${index}`} color={theme.dim}>
                  {line}
                </Text>
              ))}
              {shadowSegmentBlockReasonLines.length > 0 ? (
                <InkBox flexDirection="column" marginTop={1}>
                  <Text color={theme.dim}>Block reason</Text>
                  {shadowSegmentBlockReasonLines.map((line, index) => (
                    <Text key={`shadow-segment-block-reason-${index}`} color={shadowSegmentColor}>
                      {line}
                    </Text>
                  ))}
                </InkBox>
      ...
      <InkBox marginTop={1}>
        <Box title="Routed Shadow Evidence" width="100%" accent={routedShadowStateKnown && routedShadowTotalResolved > 0 && !routedShadowLegacyDominant}>
          <InkBox flexDirection="column" marginTop={1}>
            <Text color={theme.dim}>Routed post-segmentation evidence only. Legacy and unassigned shadow history is shown separately.</Text>
            <StatRow label="Status" value={routedShadowStatus} color={routedShadowColor} />
            <StatRow label="Threshold" value={routedShadowMinResolved > 0 ? formatNumber(routedShadowMinResolved) : '-'} color={theme.dim} />
            <StatRow label="Total resolved" value={formatNumber(routedShadowTotalResolved)} color={theme.white} />
            <StatRow label="Coverage" value={routedShadowCoverageLabel} color={routedShadowCoveragePct == null ? theme.dim : theme.white} />
            <StatRow label="Gate" value={routedShadowGateLabel} color={routedShadowGateSufficient ? theme.green : theme.yellow} />
            <StatRow label="Routed resolved" value={formatNumber(routedShadowRoutedResolved)} color={theme.white} />
            <StatRow label="Legacy resolved" value={formatNumber(routedShadowLegacyResolved)} color={theme.white} />
            <StatRow
              label="Mix"
              value={routedShadowTotalResolved > 0 ? `${formatNumber(routedShadowRoutedResolved)}/${formatNumber(routedShadowTotalResolved)} routed` : 'no routed evidence yet'}
              color={routedShadowLegacyDominant ? theme.yellow : theme.white}
            />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Summary</Text>
              {routedShadowSummaryLines.map((line, index) => (
                <Text key={`routed-shadow-summary-${index}`} color={routedShadowColor}>
                  {line}
                </Text>
              ))}
              {routedShadowDetailWrappedLines.length > 0 ? (
                <InkBox flexDirection="column" marginTop={1}>
                  <Text color={theme.dim}>Warnings</Text>
                  {routedShadowDetailWrappedLines.map((line, index) => (
                    <Text key={`routed-shadow-detail-${index}`} color={routedShadowColor}>
                      {line}
                    </Text>
                  ))}
                </InkBox>
      ...
      <InkBox marginTop={1}>
        <Box title="Routed Shadow Epoch" width="100%" accent={routedShadowEpochKnown && routedShadowEpochStartedAt > 0}>
          <InkBox flexDirection="column" marginTop={1}>
            <Text color={theme.dim}>Shadow evidence epoch only. Source and active scope labels describe the evidence window, not the live runtime state.</Text>
            <StatRow label="Status" value={routedShadowEpochStatus} color={routedShadowEpochColor} />
            <StatRow label="Started" value={routedShadowEpochStartedAt > 0 ? formatSettingsDateTime(routedShadowEpochStartedAt) : '-'} color={theme.white} />
            <StatRow label="Source" value={routedShadowEpochSourceLabel || '-'} color={theme.white} />
            <StatRow label="Active scope" value={routedShadowEpochActiveScopeLabel || '-'} color={theme.white} />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Summary</Text>
              {routedShadowEpochSummaryLines.map((line, index) => (
                <Text key={`routed-shadow-epoch-summary-${index}`} color={routedShadowEpochColor}>
                  {line}
                </Text>
              ))}
            </InkBox>
      ...
      <InkBox marginTop={1}>
        <Box title="Routed Shadow Performance" width="100%" accent={routedShadowPerformanceStateKnown && routedShadowPerformanceReady}>
          <InkBox flexDirection="column" marginTop={1}>
            <Text color={theme.dim}>Routed post-segmentation performance only. Legacy and unassigned shadow history is excluded from this box.</Text>
            <Text color={theme.dim}>P&amp;L / bankroll uses routed-only P&amp;L over the global shadow bankroll, not a standalone routed bankroll.</Text>
            <StatRow label="Status" value={routedShadowPerformanceStatus} color={routedShadowPerformanceColor} />
            <StatRow label="Gate" value={routedShadowGateLabel} color={routedShadowGateSufficient ? theme.green : theme.yellow} />
            <StatRow label="Treat as sufficient" value={routedShadowPerformanceReady ? 'yes' : 'no'} color={routedShadowPerformanceReady ? theme.green : theme.yellow} />
            <StatRow label="Pnl" value={routedShadowTotalPnlUsd == null ? '-' : `$${formatNumber(routedShadowTotalPnlUsd)}`} color={routedShadowTotalPnlUsd == null ? theme.dim : routedShadowTotalPnlUsd >= 0 ? theme.green : theme.red} />
            <StatRow label="P&L / bankroll" value={formatSettingsPercent(routedShadowReturnPct)} color={routedShadowReturnPct == null ? theme.dim : routedShadowReturnPct >= 0 ? theme.green : theme.red} />
            <StatRow label="Profit factor" value={formatNumber(routedShadowProfitFactor, 2)} color={routedShadowProfitFactor == null ? theme.dim : routedShadowProfitFactor >= 1 ? theme.green : theme.red} />
            <StatRow label="Expectancy" value={routedShadowExpectancyUsd == null ? '-' : `$${formatNumber(routedShadowExpectancyUsd)}`} color={routedShadowExpectancyUsd == null ? theme.dim : routedShadowExpectancyUsd >= 0 ? theme.green : theme.red} />
            <StatRow label="Total resolved" value={formatNumber(routedShadowTotalResolved)} color={theme.white} />
            <StatRow label="Coverage" value={routedShadowCoverageLabel} color={routedShadowCoveragePct == null ? theme.dim : theme.white} />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Summary</Text>
              {routedShadowPerformanceSummaryLines.map((line, index) => (
                <Text key={`routed-shadow-performance-summary-${index}`} color={routedShadowPerformanceColor}>
                  {line}
                </Text>
              ))}
              {routedShadowPerformanceDetailWrappedLines.length > 0 ? (
                <InkBox flexDirection="column" marginTop={1}>
                  <Text color={theme.dim}>Warnings</Text>
                  {routedShadowPerformanceDetailWrappedLines.map((line, index) => (
                    <Text key={`routed-shadow-performance-detail-${index}`} color={routedShadowPerformanceColor}>
                      {line}
                    </Text>
                  ))}
                </InkBox>
      ...
      {dbIntegrityKnown && !dbIntegrityOk ? (
        <InkBox marginTop={1}>
          <Box title="Database Integrity" width="100%">
            <InkBox flexDirection="column" marginTop={1}>
              <StatRow label="Status" value={dbIntegrityStatus} color={dbIntegrityColor} />
              {dbIntegrityLines.map((line, index) => (
                <Text key={`db-integrity-${index}`} color={dbIntegrityColor}>
                  {line}
                </Text>
              ))}
            </InkBox>
      ...
      <InkBox marginTop={1}>
        <Box title="Database Recovery" width="100%" accent={dbRecoveryStateKnown && dbRecoveryCandidateReady && !dbRecoveryPendingState}>
          <InkBox flexDirection="column" marginTop={1}>
            ...
            <StatRow
              label="Use"
              value={dbRecoveryCandidateModeLabel}
              color={dbRecoveryCandidateModeColor}
            />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Summary</Text>
              {dbRecoverySummaryLines.map((line, index) => (
                <Text key={`db-recovery-summary-${index}`} color={dbRecoveryColor}>
                  {line}
                </Text>
              ))}
              {dbRecoveryDetailWrappedLines.length > 0 ? (
                <InkBox flexDirection="column" marginTop={1}>
                  <Text color={theme.dim}>Details</Text>
                  {dbRecoveryDetailWrappedLines.map((line, index) => (
                    <Text key={`db-recovery-detail-${index}`} color={dbRecoveryColor}>
                      {line}
                    </Text>
                  ))}
                </InkBox>
      ...
      <InkBox marginTop={1}>
        <Box title="Recovery Candidate Shadow" width="100%" accent={dbRecoveryShadowStateKnown && dbRecoveryCandidateMode === 'evidence_ready' && !dbRecoveryPendingState}>
          <InkBox flexDirection="column" marginTop={1}>
            <Text color={theme.dim}>This panel measures whether the recovery candidate is evidence-ready. A verified backup alone is only integrity-only.</Text>
            <StatRow label="Status" value={dbRecoveryShadowStatus} color={dbRecoveryShadowColor} />
            <StatRow label="Use" value={dbRecoveryCandidateModeLabel} color={dbRecoveryCandidateModeColor} />
            <StatRow label="Evaluation" value={dbRecoveryShadowEvaluationLabel} color={dbRecoveryShadowMigratedEvaluation ? theme.yellow : theme.white} />
            <StatRow label="Candidate" value={dbRecoveryShadowCandidatePath || 'no candidate'} color={dbRecoveryShadowCandidatePath ? theme.white : theme.dim} />
            <StatRow label="Acted" value={formatNumber(dbRecoveryShadowActed)} color={theme.white} />
            <StatRow label="Resolved" value={formatNumber(dbRecoveryShadowResolved)} color={theme.white} />
            <StatRow
              label="PnL"
              value={dbRecoveryShadowTotalPnlUsd == null ? '-' : `$${formatNumber(dbRecoveryShadowTotalPnlUsd)}`}
              color={dbRecoveryShadowTotalPnlUsd == null ? theme.dim : dbRecoveryShadowTotalPnlUsd >= 0 ? theme.green : theme.red}
            />
            <StatRow
              label="Return"
              value={formatSettingsPercent(dbRecoveryShadowReturnPct)}
              color={dbRecoveryShadowReturnPct == null ? theme.dim : dbRecoveryShadowReturnPct >= 0 ? theme.green : theme.red}
            />
            <StatRow
              label="Profit factor"
              value={formatNumber(dbRecoveryShadowProfitFactor, 2)}
              color={dbRecoveryShadowProfitFactor == null ? theme.dim : dbRecoveryShadowProfitFactor >= 1 ? theme.green : theme.red}
            />
            <StatRow
              label="Expectancy"
              value={dbRecoveryShadowExpectancyUsd == null ? '-' : `$${formatNumber(dbRecoveryShadowExpectancyUsd)}`}
              color={dbRecoveryShadowExpectancyUsd == null ? theme.dim : dbRecoveryShadowExpectancyUsd >= 0 ? theme.green : theme.red}
            />
            <StatRow label="Segments ready" value={`${formatNumber(dbRecoveryShadowSegmentReadyCount)}/${formatNumber(dbRecoveryShadowSegmentTotal)}`} color={theme.white} />
            <StatRow label="Segments blocked" value={formatNumber(dbRecoveryShadowSegmentBlockedCount)} color={dbRecoveryShadowSegmentBlockedCount > 0 ? theme.red : theme.dim} />
            <InkBox flexDirection="column" marginTop={1}>
              <Text color={theme.dim}>Summary</Text>
              {dbRecoveryShadowSummaryLines.map((line, index) => (
                <Text key={`db-recovery-shadow-summary-${index}`} color={dbRecoveryShadowColor}>
                  {line}
                </Text>
              ))}
              {dbRecoveryShadowMigrationNote ? (
                <Text color={theme.dim}>{dbRecoveryShadowMigrationNote}</Text>
              ) : null}
              {dbRecoveryShadowDetailWrappedLines.length > 0 ? (
                <InkBox flexDirection="column" marginTop={1}>
                  <Text color={theme.dim}>Warnings</Text>
                  {dbRecoveryShadowDetailWrappedLines.map((line, index) => (
                    <Text key={`db-recovery-shadow-detail-${index}`} color={dbRecoveryShadowColor}>
                      {line}
                    </Text>
                  ))}
                </InkBox>
              ) : null}
              {dbRecoveryShadowSegmentSummaryLines.length > 0 ? (
                <InkBox flexDirection="column" marginTop={1}>
                  <Text color={theme.dim}>Segment summary</Text>
                  {dbRecoveryShadowSegmentSummaryLines.map((line, index) => (
                    <Text key={`db-recovery-shadow-segment-${index}`} color={theme.dim}>
                      {line}
                    </Text>
                  ))}
                </InkBox>
