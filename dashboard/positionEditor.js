...
function ensurePositionEditTables(db) {
    db.exec(`
...
export function savePositionManualEdit(input) {
    ...
    `);
        const transaction = db.transaction(() => {
            if (input.sourceKind === 'position') {
                savePositionEdit.run(marketId, tokenId, side, realMoney, entryPrice, shares, sizeUsd, status, nowTs);
            }
            if (input.sourceTradeLogId != null) {
                saveTradeLogEdit.run(input.sourceTradeLogId, entryPrice, shares, sizeUsd, status, nowTs);
            }
        });
        transaction();
    }
